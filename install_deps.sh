...
for repo in abseil-cpp benchmark config core-math gipfeli googletest lodepng multiprecision protobuf re2 zfp; do
  if [ ! -d "$repo" ]; then
...
for repo in googletest abseil-cpp benchmark config gipfeli multiprecision protobuf re2 zfp core-math; do
  pushd "$repo"
  ...
  if [ "${AGENT_OS?}" == "Darwin" ]; then
    ...
    # The Abseil x86 unwinder wants to track the frames using rbp, see
