...
class EclipseTest : public testing::Test {
 protected:
  ...
  TEST_F(EclipseTest, Dummy) {
    ...
    // Earth/Moon lineup for U23
    beta = ArcCos(InnerProduct(q_U23 - q_earth, q_U23 - q_moon) / ((q_U23 - q_moon).Norm() * (q_U23 - q_earth).Norm()));
    // Do the angles work?
    LOG(ERROR) << alpha;
    LOG(ERROR) << beta;
    EXPECT_THAT(AbsoluteError(alpha, beta), AllOf(Lt(1.0 * Milli(Radian)), Gt(1.0 * Nano(Radian))));
    ...
    // Earth/Moon lineup for U14
    gamma = ArcCos(InnerProduct(q_U14 - q_earth, q_U14 - q_moon) / ((q_U14 - q_moon).Norm() * (q_U14 - q_earth).Norm()));
    // Do the angles work?
    LOG(ERROR) << alpha;
    LOG(ERROR) << gamma;
    EXPECT_THAT(AbsoluteError(alpha, gamma), AllOf(Lt(1.0 * Milli(Radian)), Gt(1.0 * Nano(Radian))));
