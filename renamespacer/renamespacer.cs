...
class Emendator {
  ...
  public static void FixCompatibilityNamespace(Parser.File file) {
  ...
  public static void FixFileUsingDirective(Parser.File file) {
  ...
  public static void FixLegacyInternalNamespaces(Parser.File file) {
  ...
  public static void FixUselessInternalNamespace(Parser.File file) {
    var legacy_internal_namespaces = FindLegacyInternalNamespaces(file);
    foreach (Namespace internal_namespace in legacy_internal_namespaces) {
      var parent = internal_namespace.parent;
      Debug.Assert(parent is Namespace,
  ...
  static void Main(string[] args) {
    ...
    foreach (DirectoryInfo client in clients) {
      ...
      foreach (FileInfo input_file in all_client_files) {
        ...
        Emendator.FixUsingDeclarations(parser_file,
    ...
    foreach (FileInfo input_file in hpp_files) {
      ...
      Parser.File parser_file = file_info_to_file[input_file];
      Emendator.FixLegacyInternalNamespaces(parser_file);
      Emendator.FixMissingInternalNamespaces(parser_file,
                                            insert_using_declarations: true);
      Emendator.FixCompatibilityNamespace(parser_file);
      RewriteFile(input_file, parser_file, dry_run);
    }
    ...
    foreach (FileInfo input_file in all_body_files) {
      ...
      if (IsTest(input_file)) {
        Emendator.FixUselessInternalNamespace(parser_file);
        Emendator.FixFileUsingDirective(parser_file);
      } else {
        Emendator.FixLegacyInternalNamespaces(parser_file);
        Emendator.FixMissingInternalNamespaces(parser_file,
