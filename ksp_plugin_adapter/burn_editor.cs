...
class BurnEditor : ScalingRenderer {
  public BurnEditor(PrincipiaPluginAdapter adapter,
    ...
    Δv_tangent_ =
        new DifferentialSlider(label            : Localizer.Format("#Principia_BE_Label_DV1"),
                               unit             : "m / s",
                               log10_lower_rate : Log10ΔvLowerRate,
                               log10_upper_rate : Log10ΔvUpperRate,
                               text_colour      : Style.Tangent);
    Δv_normal_ =
        new DifferentialSlider(label            : Localizer.Format("#Principia_BE_Label_DV2"),
                               unit             : "m / s",
                               log10_lower_rate : Log10ΔvLowerRate,
                               log10_upper_rate : Log10ΔvUpperRate,
                               text_colour      : Style.Normal);
    Δv_binormal_ =
        new DifferentialSlider(label            : Localizer.Format("#Principia_BE_Label_DV3"),
                               unit             : "m / s",
                               log10_lower_rate : Log10ΔvLowerRate,
                               log10_upper_rate : Log10ΔvUpperRate,
                               text_colour      : Style.Binormal);
    previous_coast_duration_ = new DifferentialSlider(
        label            : Localizer.Format("#Principia_BE_Label_Tinitial"),
        unit             : null,
    ...
    reference_frame_selector_ = new ReferenceFrameSelector(
                                    ...
                                    Localizer.Format("#Principia_BE_FrameSelector"));
  ...
  public Event Render(string header,
    ...
    using (new UnityEngine.GUILayout.HorizontalScope()) {
      ...
      if (!minimized &&
          !reference_frame_selector_.FrameParameters().Equals(
              adapter_.plotting_frame_selector_.FrameParameters())) {
        info = Localizer.Format("#Principia_BE_Info_MF");
      }
      UnityEngine.GUILayout.Label(
          info,
          Style.Info(UnityEngine.GUI.skin.label));
      if (UnityEngine.GUILayout.Button(Localizer.Format("#Principia_BE_Button_delete"), GUILayoutWidth(2))) {
    ...
    using (new UnityEngine.GUILayout.VerticalScope()) {
      ...
      if (anomalous) {
        ...
        using (new UnityEngine.GUILayout.HorizontalScope()) {
          if (UnityEngine.GUILayout.Button(Localizer.Format("#Principia_BE_Button_ActiveEngine"))) {
            engine_warning_ = "";
            ComputeEngineCharacteristics();
            changed = true;
          } else if (UnityEngine.GUILayout.Button(Localizer.Format("#Principia_BE_Button_ActiveRcs"))) {
            engine_warning_ = "";
            ComputeRCSCharacteristics();
            changed = true;
          } else if (UnityEngine.GUILayout.Button(Localizer.Format("#Principia_BE_Button_InstantImpulse"))) {
            engine_warning_ = "";
            UseTheForceLuke();
            changed = true;
          }
      ...
      if (is_inertially_fixed_ !=
          UnityEngine.GUILayout.Toggle(is_inertially_fixed_,
                                       Localizer.Format("#Principia_BE_Button_InertiallyFixed"))) {
        changed = true;
      ...
      UnityEngine.GUILayout.Label(
          index == 0 ? Localizer.Format("#Principia_BE_Text_Start1")
                     : Localizer.Format("#Principia_BE_Text_Start2", index),
          style : new UnityEngine.GUIStyle(UnityEngine.GUI.skin.label){
              alignment = UnityEngine.TextAnchor.UpperLeft});
      using (new UnityEngine.GUILayout.HorizontalScope()) {
        UnityEngine.GUILayout.Label(
            Localizer.Format("#Principia_BE_Text_Menauvere") + Δv().ToString("0.000") + " m/s",
            GUILayoutWidth(8));
        UnityEngine.GUILayout.Label(Localizer.Format("#Principia_BE_Text_Durationg") + duration_.ToString("0.0") +
                                    " s");
      }
  ...
  private void ComputeEngineCharacteristics() {
    ...
    if (thrust_in_kilonewtons_ == 0) {
      engine_warning_ += Localizer.Format("#Principia_BE_Warm_NoActionEngine");
      ComputeRCSCharacteristics();
    }
  }
  ...
  private void ComputeRCSCharacteristics() {
    ...
    if (thrust_in_kilonewtons_ == 0) {
      engine_warning_ += Localizer.Format("#Principia_BE_Warm_NoActionRCS");
      UseTheForceLuke();
    }
  }
