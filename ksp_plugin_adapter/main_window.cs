...
internal class MainWindow : VesselSupervisedWindowRenderer {
  ...
  private const string next_release_name_ = "Germain";
  private const int next_release_lunation_number_ = 260;
  private readonly DateTimeOffset next_release_date_ =
      new DateTimeOffset(2021, 01, 13, 05, 01, 00, TimeSpan.Zero);
    ...
    using (new UnityEngine.GUILayout.VerticalScope()) {
      if (!adapter_.PluginRunning()) {
        UnityEngine.GUILayout.Label(
            text : Localizer.Format("#Principia_MainMenu_WarnText"),
      ...
      if (DateTimeOffset.Now > next_release_date_) {
        if (Versioning.version_minor <= 7) {
          ...
          UnityEngine.GUILayout.TextArea(
              Localizer.Format("#Principia_MainMenu_VersionInformText2",
              next_release_lunation_number_,
              next_release_name_ ),
              style: Style.Multiline(UnityEngine.GUI.skin.textArea));
        }
      ...
      if (MapView.MapIsEnabled &&
        ...
        using (new UnityEngine.GUILayout.HorizontalScope()) {
          ...
          if (FlightGlobals.fetch.VesselTarget?.GetVessel()) {
            ...
            if (UnityEngine.GUILayout.Button(Localizer.Format("#Principia_MainMenu_Button_Switch"))) {
              var focus_object =
