...
void JournalProtoProcessor::ProcessRepeatedMessageField(FieldDescriptor const* descriptor) {
  cpp_field_type_[descriptor] = descriptor->message_type()->name() +
    " const*";
}

void JournalProtoProcessor::ProcessOptionalInt32Field(FieldDescriptor const* descriptor) {
  cpp_field_type_[descriptor] = "int const*";
}

void JournalProtoProcessor::ProcessRequiredFixed64Field(FieldDescriptor const* descriptor) {
  ...
  field_serializer_wrapper[descriptor] =
    [](std::string const& expr) {
    return "SerializePointer(" + expr + ")";
  };
}

void JournalProtoProcessor::ProcessRequiredMessageField(FieldDescriptor const* descriptor) {
  ...
  field_copy_wrapper_[descriptor] =
    ...
    return "*" + name + "mutable_" + descriptor->name() + "() = " + expr;
  };
  field_serializer_wrapper[descriptor] =
    [](std::string const& expr) {
...
void JournalProtoProcessor::ProcessRequiredBoolField(FieldDescriptor const* descriptor) {
  cpp_field_type_[descriptor] = descriptor->cpp_type_name();
  cpp_field_setter_[descriptor] = "set_" + descriptor->name();
}

void JournalProtoProcessor::ProcessRequiredDoubleField(FieldDescriptor const* descriptor) {
  cpp_field_type_[descriptor] = descriptor->cpp_type_name();
  cpp_field_setter_[descriptor] = "set_" + descriptor->name();
}

void JournalProtoProcessor::ProcessRequiredInt32Field(FieldDescriptor const* descriptor) {
  cpp_field_type_[descriptor] = "int";
  cpp_field_setter_[descriptor] = "set_" + descriptor->name();
}

void JournalProtoProcessor::ProcessSingleStringField(FieldDescriptor const* descriptor) {
  cpp_field_type_[descriptor] = "char const*";
  cpp_field_setter_[descriptor] = "set_" + descriptor->name();
}

void JournalProtoProcessor::ProcessOptionalField(FieldDescriptor const* descriptor) {
...
void JournalProtoProcessor::ProcessRepeatedField(FieldDescriptor const* descriptor) {
  switch (descriptor->type()) {
...
void JournalProtoProcessor::ProcessRequiredField(FieldDescriptor const* descriptor) {
  switch (descriptor->type()) {
  ...
  if (in_out_field_.find(descriptor) != in_out_field_.end()) {
    ...
    in_out_field_wrapper[descriptor] = [](std::string const& expr) {
      return "*" + expr;
    }
  }
}

void JournalProtoProcessor::ProcessSingleField(FieldDescriptor const* descriptor) {
...
void JournalProtoProcessor::ProcessField(FieldDescriptor const* descriptor) {
  ...
  in_out_field_wrapper[descriptor] =
    [](std::string const& expr) {
    return expr;
  };
  optional_field_wrapper_[descriptor] =
    ...
    cpp_fill_body_[descriptor] +=
      ...
      optional_field_wrapper_[descriptor](
        ...
        field_copy_wrapper_[descriptor](
          "m->",
          field_serializer_wrapper[descriptor](
            in_out_field_wrapper_[descriptor](field_name)))) + "\n";
...
void JournalProtoProcessor::ProcessReturn(Descriptor const* descriptor) {
  ...
  ProcessField(field_descriptor);
  cpp_fill_body_[descriptor] =
    "  auto* const m = message->mutable_return_();\n  " +
    cpp_field_copy_[field_descriptor] + "\n";
  cpp_nested_type_[descriptor] =
    "  using Return = " + cpp_field_type_[field_descriptor] + ";\n";
}

void JournalProtoProcessor::ProcessMethodExtension(Descriptor const* descriptor) {
  std::string const& name = descriptor->name();
  bool has_in = false;
  bool has_out = false;
