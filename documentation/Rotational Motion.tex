...
\label{eqnFBN}
\end{equation}
where we have defined $\vr \DefineAs \vq_2 - \vq_1$.  Since we'll need to use $\ddot{\vr}$ later, it is important to note here that $\ddot{\vr} = \ddot{\vq_2} - \ddot{\vq_1}$ where $\ddot{\vq_1}$ is the acceleration exerted on $B_1$ by the rest of the system (and similarly, $\ddot{\vq_2}$ is the acceleration exerted on $B_2$ by the rest of the system).
