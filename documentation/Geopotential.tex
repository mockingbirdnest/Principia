...
The value of $\cos^m \gb$ may be computed by recurrence in a way that entails $\BigO\of{log_2\of m}$ rounding errors.  If $m$ is even, we compute:
\[
\begin{dcases}
h = \frac m 2 \\
...
\end{dcases}
\]
The value of $\cos m \gl$ and $\sin m \gl$ may similarly be computed using elementary trigonometric identities with $\BigO\of{log_2\of m}$ rounding errors.  If $m$ is even, we compute:
\[
\begin{dcases}
h = \frac m 2 \\
...
\end{dcases}
\]
For the Legendre polynomials, we are looking for a recurrence relationship among terms of the form $\deriv[m]{t}{\LegendrePolynomial{n}\of t}$.  First note that these terms are $0$ when $m > n$.  We start for the recurrence defining the polynomials themselves:
\[
...
so we end up with:
\[
n \LegendrePolynomial{n}\of t = \pa{2 n - 1}\pa{t \deriv[m]{t}{\LegendrePolynomial{n - 1}\of t} + m \deriv[m - 1]{t}{\LegendrePolynomial{n - 1}\of t}} -
\pa{n - 1} \LegendrePolynomial{n - 2}\of t
\]
This recurrence starts with ${\LegendrePolynomial{0}\of t} = 1$, ${\LegendrePolynomial{1}\of t} = t$ and ${\deriv{t}{\LegendrePolynomial{1}\of t}} = 1$.  It is similar to a relation derived by \cite[appendix]{Westra2017} and makes it possible to efficiently evaluate $\Evaluate{\deriv[m]{t}{\LegendrePolynomial{n}\of t}}{t = \sin \gb}$ for increasing values of $n$ and $m$.
\printbibliography
\end{document}