...
\]
We are going to construct a value of $k$ such that the smallest root $r_-$ is in $\intopen{s_0}{s_1}$.  First, consider the inequality $r_-<s_1$.  It can be rewritten:
\begin{align*}
&6 s_0\pa{k+2} - 6\pa{s_0+s_1} < \sqrt{6\pa{k+2}\pa{k\pa{s_1^2-2 s_0 s_1 + 3s_0^2} + 3\pa{s_1-s_0}^2}} \\
\iff &6\pa{k s_0 + \pa{s_0-s_1}}^2 < \pa{k+2}\pa{k\pa{s_1^2-2 s_0 s_1 + 3s_0^2} + 3\pa{s_1-s_0}^2} \\
...
\iff &k < \frac{5s_1-3s_0}{3s_0-s_1}
\end{align*}
When $s_1$ is sufficiently close to $3s_0$, let's write $s_1=3\pa{1-\ge}s_0$ with $\ge>0$.  The above inequality becomes:
\[
k < \frac{15\pa{1-\ge}-3}{3\ge} = \frac{4}{\ge} + \BigO\of{1}
\]
...
\end{align*}
Because the polynomial in $k$ on the left-hand side of this inequality has a coefficient of $k^2$ which is positive when $s_1<3s_0$, the inequality is true if and only if $k$ is   greater that the largest root the polynomial.  Using $s_1=3\pa{1-\ge}s_0$ the roots can be found by solving:
\[
9 k^2\pa{\ge + \BigO\of{\ge^2}} - 12 k\pa{1 + \BigO\of\ge} - 16 \pa{1 + \BigO\of\ge} = 0
\]
Which yields the condition on $k$:
\[
k > \frac{4}{3\ge} + \BigO\of{1}
\]
