...
\grad \gs\mathfrak R=\pa{\gs'\mathfrak R + \gs\mathfrak R'} \frac \vr r\text.
\]

\subsection*{Sigmoid}
Since we want the force to remain continuous, we need $\gs$ to be $\Cont[1]$; we achieve this by
making $\gs$ the cubic interpolation on the non-constant interval $\intclos {s_0} {s_1}$,
\[
\gs\of r=
\begin{cases}
...
\end{cases}
\]
Further, we require that the radial component of the force resulting from the damped $V_{nm}$ and all
its derivatives be monotone, i.e., that
\[
\forall k \in \Positives, \deriv[k] r {\InnerProduct {\grad V_{nm}}{\frac \vr r}} = \deriv[k+1] r {\gs\mathfrak R\of r}{\mathfrak B\of \gb}\,{\mathfrak L\of \gl}
\]
have constant sign.
...
\]
We have \[
\max_\gl \abs{\,\mathfrak L\of\gl} = \max_\gl \abs{C_{nm}\cos m\gl + S_{nm} \sin m\gl} = \sqrt{C_{nm}^2 + S_{nm}^2}\text,
\]
