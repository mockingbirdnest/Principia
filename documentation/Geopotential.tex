...
\]
We require that it and all its derivatives be monotone, \idest, that
\[
...
\end{align*}
When $n=2$ equation (\ref{eqn3s0}) further reduces to a first-degree equation with root $r = \frac{3}{2} s_0 \pa{k+2}$ which verifies $r≥3s_0$.  Note however that the latter inequality is not strict, \idest, the value $3s_0$ may be reached if $k=0$ and $n=2$.
\end{proof}
