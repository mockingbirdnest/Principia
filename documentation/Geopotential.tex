...
\end{align*}
The following lemma gives us a sufficient condition for the proposition above:
\begin{lemma}
If $s_1 = 3s_0$ then the positive roots of $d_{n k}\of{s_0, s_1; r}$ are greater than $s_1$.
\end{lemma}
\begin{proof}
With this choice of $s_1$ the constant term of $d_{n k}\of{s_0, s_1; r}$ vanishes and $r=0$ is a trivial root.  We are left with the equation:
\[
...
\]
We want to prove that the smallest root, $r_-$ verifies $r_{-}>0$ and $r_{-}>s_0$.  The first inequality can be rewritten:
\begin{align*}
&\pa{n+k-1}\pa{n-1} > \sqrt{\pa{n+k-1}\pa{n-1}\pa{k+1}} \\
\iff &\pa{n+k-1}\pa{n-1} > k + 1 \\
\iff &\pa{n+k} \pa{n-2} > 0 \quad \text{which is trivally true.}
\end{align*}
...
\end{align*}
When $n=2$ equation (\ref{eqn3s0}) further reduces to a first-degree equation with root $r = \frac{3}{2} s_0 \pa{k+2}$ which verifies $r>0$ and $r≥3s_0$.  Note however that the latter inequality is not strict, i.e., the value $3s_0$ may be reached if $k=0$ and $n=2$.
