...
The value of $\cos^m \gb$ may be computed by recurrence.  If $m$ is even, we have:
\[
\begin{dcases}
h = \frac m 2 \\
...
\end{dcases}
\]
The value of $\cos m \gl$ and $\sin m \gl$ may similarly be computed using elementary trigonometric identities with an absolute error of $\BigO\of {m^2}$ ulps.  If $m$ is even, we have:
\[
\begin{dcases}
h = \frac m 2 \\
