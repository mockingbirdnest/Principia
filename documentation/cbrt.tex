...
with $\max_{y}\abs{ε'} \approx 0.03155$,
...
\]
Note that the subtraction in the numerator is exact by Sterbenz's lemma \cite[p.~138, theorem~4.3.1]{Sterbenz1974}.
...
$\max_{y} \abs{Δ'} \approx 0.00002196$, $\log_2 \max_{y} \abs{Δ'} \approx -15.47$.
...
\end{align*}
with $\max_{y} \abs{ε'} \approx 0.0032025$, but
$\max_{y} \abs{Δ'} \approx 0.00002086$, $\log_2 \max_{y} \abs{Δ'} \approx -15.54$.
The corresponding fixed-point constant is\[C_{\mathrm{L}}\DefineAs\hex{2A9F\,7893\,782D\,A1CE} \Multiply 2^{-52}\]for binary64.
\marginpar{\small
TODO(egg): $Δ'$ accounts for more than half of the excess above correct rounding in the final bound (for $Δ=0$, the bound becomes $1.0001832u$). Consider using either
...
It is fairly clear that $Δ'$ dominates the rounding error in the approximation
$Δ\approx Δ'$; for the sake of completeness we quantify this.
...
\abs{\frac{ξ}{ξ'}-1}<
\frac{1+ε'}{1+Δ'}
\pa{γ_3+\frac{\pa{1+γ_4}γ_{11}\pa{1+ε'}^3+γ_{16}\pa{\pa{1+ε'}^3-1}}{(\pa{1+ε'}^3+1)+\pa{1-γ_{12}}}}\text.
\]
Linearizing, this bound is $\pa{6+\tfrac{2}{3} + \BigO\of{ε'+Δ'}}u+ \BigO\of{u^2}$.
...
overall $
\frac{1+γ_{7}}{1-γ_{5}}-1
$ on the correction term.

Since the relative error of $x$ is at most $\pa{1+Δ}\pa{1+2^{-16}}-1$, the rounding-free
correction term is no larger than $\pa{\pa{1+Δ}\pa{1+2^{-16}}-1}\cuberoot y$, so that the absolute error of the computed correction term is bounded by\[
\pa{\frac{1+γ_{7}}{1-γ_{5}}-1}\pa{\pa{1+Δ}\pa{1+2^{-16}}-1}\cuberoot y\text.
\]
This gives us our bound for the relative error of the result,\[
\abs{\frac{r}{\cuberoot y}-1}<\pa{1+\pa{\frac{1+γ_{7}}{1-γ_{5}}-1}\pa{\pa{1+Δ}\pa{1+2^{-16}}-1}}\pa{1+u}-1\text.
\]
