...
than monic polynomials of degree $p$, by defining $E_p$ and $E_{p-1}$ in terms of Taylor polynomials
...
We call this method the \emph{generalized Lagny method of degree $d$ and order $p$}.
...
the generalized methods of high order can thus be applied to polynomials of low degree.
...
and consider the polynomials in $Δ$\[
\tilde E_p \DefineAs M_p\text{ and }\tilde E_{p-1} \DefineAs M_{p-1}\text.\]
Let $\tilde E_{n-1}$ be the remainder of the polynomial of $\tilde E_{n+1}$ by $\tilde E_{n}$.
...
the (quadratic) irrational method for the cubic to arbitrary polynomials, retaining cubic convergence; when $f$ is
