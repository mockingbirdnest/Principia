...
\end{equation}
where\[
T_{n}\DefineAs\sum{k=0}[n]\frac{f^{\pa{n}}\of{a}}{\Factorial k}\pa{x-\tfrac{1}{2}a}^k\text.
\]
...
Let\[
M_n \DefineAs\sum{k=0}[n] \frac{f^{\pa{n}}\of{a}}{\Factorial k}Δ^k\text,\]
...
Halley's irrational method is defined as a root of $M_2$.
...
We now turn to the computation of the cube root of $y$ in \texttt{numerics/cbrt.cpp}.
...
\item $\roundTowardPositive{α}$ for directed rounding toward positive infinity---roundTowardPositive---;
\item $\roundTowardNegative{α}$ for directed rounding toward negative infinity---roundTowardNegative---;
\item $\roundTowardZero{α}$ for directed rounding toward $0$---roundTowardZero.
\end{itemize}
...
is to use Veltkamp's method TODO CITE to round to the nearest number with the desired
number of bits; see also \cite[241]{Dekker1971} in \emph{mul12}:\[
...
\[
Δ=\round{
\frac{\round{\pa{x^3 - y} \round{\round{\round{\round{10x^3} + 16y}x^3} + \round{y^2}}}}
...
$2^{-75}$. We will not seek to bound the truncation error more closely, nor to tweak the constants in
...
\hline
$\hphantom{\leftRound{\pa{x^3-y}\leftRound{\leftRound{\leftRound{\round{10x^3}+16y}}}}}
\mathllap{\round{10x^3} + 16y}$ &
$ι^{3}\frac{10u}{26} < γ_{\frac{10}{26}ι^{3}}$ \\
$\hphantom{\leftRound{\pa{x^3-y}\leftRound{\leftRound{\round{\round{10x^3}+16y}}}}}
\mathllap{\round{\round{10x^3} + 16y}}$ &
$γ_{\frac{10}{26}ι^{3}+1}$ \\
$\hphantom{\leftRound{\pa{x^3-y}\leftRound{\round{\round{\round{10x^3}+16y}x^3}}}}
\mathllap{\round{\round{\round{10x^3} + 16y}x^3}}$ &
$γ_{\frac{10}{26}ι^{3}+2}$ \\
$\hphantom{\leftRound{\pa{x^3-y}\leftRound{\round{\round{\round{10x^3}+16y}x^3}+\round{y^2}}}}
...
$\hphantom{\leftRound{\pa{x^3-y}\round{\round{\round{\round{10x^3}+16y}x^3}+\round{y^2}}}}
...
We may build a similar {\xsuxfont 𒋛𒄥𒊏𒋫}\marginnote{Transcription:\hfill\foreign{siqqurrata}. REMOVE BEFORE FLIGHT: do we want the
akk-x-neoass spelling {\xsuxfont 𒍨𒄥𒋥}? That one is invariable though. The akk-x-midass spelling is also used by Šulmānu-ašarēdu~III.} for the denominator.
\begin{center}
\begin{tabular*}{\textwidth}{l @{\extracolsep{\fill}} r}
Expression                    & Bound on the rounding error\\
\hline
...
The following {\xsuxfont 𒋛𒄥𒊏𒀀𒌅}\marginnote{Transcription:\hfill\foreign{siqqurrātu}.} tighten the bounds on the
sums in the numerator and the denominator in the same way as for the fifth order method.
\begin{center}
\begin{tabular*}{\textwidth}{l @{\extracolsep{\fill}} r}
Expression                    & Bound on the rounding error\\
\hline
$\hphantom{\leftRound{\round{x\pa{x^3-y}}\leftRound{\leftRound{\leftRound{\round{5x^3}+\round{17y}}}}}}
\mathllap{\round{5x^3}+\round{17y}}$ & $γ_1$\\
$\hphantom{\leftRound{\round{x\pa{x^3-y}}\leftRound{\leftRound{\round{\round{5x^3}+\round{17y}}}}}}
\mathllap{\round{\round{5x^3}+\round{17y}}}$ &
$γ_2$\\
$\hphantom{\leftRound{\round{x\pa{x^3-y}}\leftRound{\round{\round{\round{5x^3}+\round{17y}}x^3}}}}
...
$\hphantom{\leftRound{\round{x\pa{x^3-y}}\round{\round{\round{\round{5x^3}+\round{17y}}x^3}}+\round{5\round{y^2}}}}
\mathllap{\round{\round{\round{\round{5x^3}+\round{17y}}x^3}+\round{5\round{y^2}}}}$ &
$\frac{1}{27}γ_{76ι^6+27}$\\
$\hphantom{\leftRound{\round{x\pa{x^3-y}}\round{\round{\round{\round{5x^3}+\round{17y}}x^3}}+\round{5\round{y^2}}}}
\mathllap{\round{x\pa{x^3-y}}\round{\round{\round{\round{5x^3}+\round{17y}}x^3}+\round{5\round{y^2}}}}$ &
...
\cuberoot{y}\in\intclos{\frac{r}{1+\max\abs{ε_r}}}{\frac{r}{1-\max\abs{ε_r}}}\DefinitionOf \mathscr{I}\text.
\]
...
This criterion may be determined as follows. The difference between $r_0=\round{r}$ and
...
$\round{\cuberoot{y}}$ may be computed as $\round{r_0+2r_1}$, which is $r_0$ only if $r_1$ is below
a quarter-unit in the last place—in which case the small size of $\mathscr{I}$ ensures that it does
not contain a tie. If $\round{r_0+2r_1}\neq r_0$, we must ascertain whether the tie lies in $\mathscr I$,\[
\frac{\round{r_0+2r_1}+r_0}{2}\in\intclos{\frac{r}{1+\max\abs{ε_r}}}{\frac{r}{1-\max\abs{ε_r}}}\text,\]
subtracting $r_0$ to get rid of the unrepresentable $r$ in the right-hand side,\[
\frac{\round{r_0+2r_1}-r_0}{2}
\in\intclos{\frac{r_1-\max\abs{ε_r}r_0}{1+\max\abs{ε_r}}}{\frac{r_1+\max\abs{ε_r}r_0}{1-\max\abs{ε_r}}}\text,\]
subtracting $r_1$ to remove the cancellation,\[
\frac{\round{r_0+2r_1}-r_0}{2}-r_1
\in\intclos{-\max\abs{ε_r}\frac{r_0+r_1}{1+\max\abs{ε_r}}}{\max\abs{ε_r}\frac{r_0+r_1}{1-\max\abs{ε_r}}}\text.\]
...
\frac{\round{r_0+2r_1}-r_0}{2}-r_1\in
\intclos{
-\frac{\max\abs{ε_r}}{1+\max\abs{ε_r}}\pa{1+u}r_0}{
 \frac{\max\abs{ε_r}}{1-\max\abs{ε_r}}\pa{1+u}r_0}\text.\]
...
If we restrict ourselves to roundTiesToAway at runtime, we must relax the bounds a little further,\[
\intclos{
\round{\roundTowardNegative{-\frac{\max\abs{ε_r}}{1+\max\abs{ε_r}}\pa{1+u}}r_0}\pa{1+u}}{
 \round{\roundTowardPositive{\frac{\max\abs{ε_r}}{1-\max\abs{ε_r}}\pa{1+u}}r_0}\pa{1+u}}\text.\]

Otherwise, there may be a misrounding. The correctly-rounded result may readily be computed
using the ordinary cube root algorithm, described, \exempligratia, in Lagny's
...
Since $\mathscr{I}$ is small enough that it cannot contain both a floating-point number
and a tie; thus if $\mathscr{I}$ contains a tie, we know that
$\roundTowardZero{\cuberoot{y}}=\roundTowardZero{r}\DefinitionOf a$\text; we thus have the
first $53$ bits already. Correct rounding may be achieved by extracting a
...
Using Veltkamp's algorithm from \cite[234]{Dekker1971} to express $a^2$ as $a^2_0+a^2_1$ with
