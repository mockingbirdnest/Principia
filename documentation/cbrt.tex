...
E_{p}&\DefineAs S_1-\frac{1}{2}R\of{x+\frac{1}{2}a}\\
...
a\mapsto a + \pa{p-1}\frac{\pa{1/f}^{\pa{p-2}}\of{a}}{\pa{1/f}^{\pa{p-1}}\of{a}}\label{LagnyRationalIterate}
\end{equation}
\begin{proof}
Let $E_p = d_0 x^p + \dotsb + d_p$, $E_p = e_0 x^{p-1} + \dotsb + e_{p-1}$.
As shown in \cite[52--54]{Householder1970}, the polynomial remainders $E_k$ is are given up to a constant factor by
...
$\det \matE = 0$, \idest, $\matE$ is singular.
\begin{lemma}[Left as an exercise to the reviewer]
The matrix $\matE_1$ is singular if and only if \[
\matC\DefineAs\begin{pmatrix}
x - \frac{1}{2}a & c_0 \\
-1               & c_1     & c_0 \\
0                & c_2     &        & \ddots \\
...
\end{pmatrix}
\]is singular, where $c_0\DefineAs1$.
\begin{proof}
...
\end{proof}
\end{lemma}
The theorem follows from the lemma and theorem 4.4.2 from \cite[169]{Householder1970}:
the iterate (\ref{LagnyRationalIterate}) corresponds to Householder’s (14) with $g\Identically 1$; for that
value of $g$, theorem 4.4.2 states that (14) is equivalent to (12) from the same page, which, substituting the iterate
$\FunctionBody{a}{x+\frac{1}{2}a}$, is $\det\matC=0$. By the lemma, this is equivalent to the definition of Lagny’s rational
method.
\end{proof}
\end{theorem}

\section*{Names}
As seen in the proof of the theorem, the iterate (\ref{LagnyRationalIterate}) was described by Householder for an
arbitrary analytic functions $f$ in \cite[169]{Householder1970} (equation (14), wherein one should take $g\Identically 1$;
see also \cite{SebahGourdon2001} which explicitly gives that form).
...
Considering that a method remains the same when generalized from arbitrary polynomials to arbitrary analytic functions,
we call the iterate (\ref{LagnyRationalIterate})\begin{itemize}
