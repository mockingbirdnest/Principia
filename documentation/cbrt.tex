...
\item $\roundTowardPositive{α}$ for the smallest positive representable number $\geq α$:
rounding-direction attribute roundTowardPositive;
\item $\roundTowardNegative{α}$ for the largest negative representable number $\leq α$:
rounding-direction attribute roundTowardNegative;
\item $\roundTowardZero{α}$ for the representable number with the same sign as $α$ and the largest magnitude
$\leq \abs α$: rounding-direction attribute roundTowardZero.
\end{itemize}
...
\frac{\tilde r+r_0}{2}\in\intclos{\frac{r}{1+\max\abs{ε_r}}}{\frac{r}{1-\max\abs{ε_r}}}\text,\]
...
\intclos{
\round{\roundTowardNegative{-\frac{\max\abs{ε_r}}{1+\max\abs{ε_r}}\pa{1+\frac{2u}{1-u}}}r_0}}{
 \round{\roundTowardPositive{\frac{\max\abs{ε_r}}{1-\max\abs{ε_r}}\pa{1+\frac{2u}{1-u}}}r_0}}\text.\]
...
with\begin{align*}
τ&\DefineAs\roundTowardPositive{\frac{\max\abs{ε_r}}{1-\max\abs{ε_r}}\pa{1+\frac{2u}{1-u}}}\\
 &=\hex{DEAD\,BEEF}\text,
\end{align*}
...
Conversely the computation of $r_0$ from $x$ may use a lower-order method, since it starts from $26$ rather than $17$,
but still cannot add more than $53$.
