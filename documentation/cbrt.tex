...
ξ = \round{\round{\round{\sqrt{3}\round{q^2}}+ \round{\sqrt{\round{\round{4yq}-\round{\round{q^2}\round{q^2}}}}}}\round{\frac{\round{1/\sqrt{12}}}{q}}}\text,
\]
\emph{mutatis mutandis} with Canon optimization, has about the same latency as the rational method.
...
is used in step~\ref{HighOrder}, a subscript $\mathrm{C}$ indicates Canon optimization, and the letter $\mathrm{N}$ indicates
...
\begin{itemize}[nosep]
\item method $3^16^1$, used by Principia from pull request \href{https://github.com/mockingbirdnest/Principia/pull/1802}{\#1802},
in 2018, until it was replaced by a correctly-rounded method;
\item method $3^2_{\mathrm{C}}6^1$;
...
implemented as written; since Kahan recommends that the high-order step be of fourth order in that case, and gives two fourth order methods,
this results in two implementations, identified by the coefficient of the asymptotic error, $\mathrm Γ$ in Kahan's notation:
\begin{itemize}[nosep]
\item $\mathrm Γ=-1$,
\item $\mathrm Γ=-35/3$;
\end{itemize}\
...
\caption{Latency on various architectures, plotted against the misrounding rate.\\
$*$ The method uses FMA.\\
$\dagger$ The method is unfaithful.}
\end{adjustwidth*}
\end{figure}
