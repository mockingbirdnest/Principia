...
For both fixed- and floating-point numbers, given $α\in\R$, we write $\round{α}$ for the nearest representable number (rounding ties to even), and $\roundTowardZero{α}$ for directed
rounding towards $0$ to the fixed-point precision.
...
The difference between $r_0=\round{r}$ and the unrounded $r$ can be computed as
described in \cite[224]{Dekker1971}, $r_1\DefineAs x-r_0+Δ$, evaluated as written
...
\]
Consider the \emph{ties}, \idest, the number halfway betwen $r$ and its binary64
successor and the number halfway between $r$ and its predecessor.
