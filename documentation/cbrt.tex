...
When $f$ is a monic polynomial of degree $p$, (\ref{GeneralizedLagnyEquations}) is equivalent to (\ref{LagnyEquations}), so that
...
1827, in \cite[X]{Babbage1827}, 8th rule. Like Babbage and Schrön, we give this bit regardless of
...
For fixed-point numbers, we write $\roundTowardZero{α}$ for directed rounding towards $0$ to the fixed-point
...
$ι^{6}\frac{1}{27}\pa{26γ_{\frac{10}{26}ι^{3}+2} + u}<\frac{1}{27}γ_{10ι^{9}+\pa{2\Multiply 26+1}ι^{6}}$\\
...
We may build a similar {\xsuxfont 𒋛𒄥𒊏𒋫}\footnote{\foreign{Ziqqurrata}.} for the denominator.
\begin{center}
\begin{tabular}{l r}
Expression                    & Bound on the rounding error\\
\hline\\
...
\end{tabular}
\end{center}
For a bound of about $4.81u$ on the denominator, a more modest improvement of $\frac{5}{27}u$ over
our earlier $5u$.
