...
\paragraph{A note on the multiplicity of irrational methods.} Lagny does not require that the polynomial division be carried out
all the way to $E_2$, merely until one gets \foreign{\textfrench{une valeur d'$x$ [...] d'un degré commode}}, by which he likely means
...
$x$ is a root of $E_2$, but when it comes to computing the fifth root, the same term is used to refer to the formula where $x$ is a root of
...
The misprint remains forty years later in \cite[340]{FantetdeLagny1733}.} for $\sqrt[5]{a^5+b}$\[
\FunctionBody{a}{\tfrac{1}{2}a+\sqrt{\sqrt{\tfrac{1}{4}a^4+\tfrac{b}{5a}}-\tfrac{1}{4}a^2}}
\]
Lagny's quartic irrational method for the fifth root, whereas the quadratic irrational method for the same fifth root would be\footnote{Both are of order $5$, but the reader who wishes
...
\]
In the context of the computation of a real cube root, there is however no ambiguity; assuming one does not wish to compute a
cube root using a cube root, the only irrational method is the quadratic one.
...
$\mathscr{L}\DefineAs\setst{\tuple{\frac{k-\FractionalPart Γ}{2},k}}{k\in\set{1,2}}$ are the local extrema. We get more precisely\footnote{These may be computed formally, but the expressions are unwieldy.}\[
Γ_{\mathrm{Kahan}} \approx 0.10096\,78121\,55802\,88786\,36993\,42643\,55358\,06489\,88235\,75289\]
with $\max_{y}\abs{ε_q'} \approx 0.03155$,
...
ε_ξ' = \frac{2{ε_q'}^3+{ε_q'}^4}{3+6ε_q'+6{ε_q'}^2+2{ε_q'}^3}\text.
\]
If $q'$ is computed using $Γ=Γ_{\mathrm{Kahan}}$, we get
$\max_{y} \abs{ε_ξ'} \approx 0.00002196$, $\log_2 \max_{y} \abs{ε_ξ'} \approx -15.47$.
...
Γ_{\mathrm{L}\Q}&\DefineAs\argmin_{Γ\in\R}\max_{y}\abs{ε_ξ'}\\
&\approx 0.09918\,74615\,29855\,99525\,66149\,20761\,31234\,34720\,23067\,92759
\end{align*}
with $\max_{y} \abs{ε_q'} \approx 0.0032025$, but
$\max_{y} \abs{ε_ξ'} \approx 0.00002086$, $\log_2 \max_{y} \abs{ε_ξ'} \approx -15.54$.
The corresponding fixed-point constant is\[C_{\mathrm{L}\Q}\DefineAs\hex{2A9F\,7893\,782D\,A1CE} \Multiply 2^{-52}\]for binary64.
...
ξ = \tfrac{1}{2}q'+ \frac{1/\sqrt{12}}{q'}\sqrt{4yq'-{q'}^4}\]
...
\marginpar{\small
TODO(egg): $ε_ξ'$ accounts for more than half of the excess above correct rounding in the final bound (for $ε_ξ=0$, the bound becomes $1.0001832u$). Consider using either
a higher order iteration or Lagny's irrational formula. Also consider optimizing the constants in the method, as described in \cite{Canon2018}, reproduced below with the author’s permission.
\begingroup
\scriptsize
...
\end{quotation}
\endgroup
Note however that in the rational method for the cube root there is little room for
optimization without altering performance; the only constant is $2$, and changing that
turns an addition into a multiplication.}
...
\abs{\frac{x}{ξ}-1}\le1-\frac{1}{1+2^{-\Floor{\frac{p}{3}}+1}-2^{1-p}} < 2^{-\Floor{\frac{p}{3}}+1}\text,
\]
$2^{-16}$ in binary64.
...
It is not immediately clear that order $5$ would incur fewer roundings though, for the
same reason that it has the same performance.}
...
\]
Substituting the truncation errors and the definition of $δ$, we can
bound the relative error arising from rounding:
\[
\abs{\frac{ξ}{ξ'}-1}<
\frac{1+ε_q'}{1+ε_ξ'}
