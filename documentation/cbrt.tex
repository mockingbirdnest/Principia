...
for $f$ around $\frac{1}{2}a$:\[
E_p\DefineAs T_{p}-\tfrac{1}{2}T_{p-1}\text{ and }E_{p-1}\DefineAs \tfrac{1}{2}T_{p-1}\text,
\]
where\[
T_{n}\DefineAs\sum{k=0}[n]\frac{f^{\pa{n}}\of{\tfrac{1}{2}a}}{\Factorial k}\pa{x-\tfrac{1}{2}a}^k\text.
\]
The rest of the method remains unchanged; the iteration is given by $\FunctionBody{a}{x+\frac{1}{2}a}$ for a root $x$ of $E_{d}$.
We call this method the \emph{generalized Lagny method of degree $d$ and order $p$} when $f$ is not
a polynomial of degree $p$; we use the terms ``rational'', ``quadratic irrational'', etc. for $d=1$, $d=2$, etc. respectively.
...
These methods may equivalently be constructed using Maclaurin series in the correction term.
Let\[
M_n \DefineAs\sum{k=0}[n] \frac{f^{\pa{n}}\of{a+Δ}}{\Factorial k}Δ^k\text,\]
...
Halley likewise generalized the (quadratic) irrational method for the cubic to arbitrary\footnote{Lagny's method is general, in that an iteration is given for any
...
known to Schröder.
\end{quote}
It is possible that the irrational methods could be expressed using Schröder’s methods in one way or another, but while the result would probably be more convenient, it is unlikely to be something
well-known, as irrational methods are far less popular nowadays---unjustifiedly so, as we shall see.

Our generalization of Lagny's irrational methods to arbitrary $f$, decoupling the degree of $f$
from the convergence order, was inspired by Gander's rephrasing of Halley's construction in \cite{Gander1985}, wherein the correction term of Halley's irrational method is expressed as a
root of $M_2$.
...
\abs{\frac{Σ}{Σ'}-1}\le\pa{\frac{1+\abs{ε_x}}{1-\abs{ε_x}}}^{3n}\frac{αδ_1+βδ_2}{α+β}\text.
\]
Unfortunately $γ$ is poorly behaved under weighted averages, so we will resort to old-fashioned products of $1+u$. 
