...
all the way to $E_2$, merely until one gets \foreign{\textfrench{une valeur d'$x$ rationelle \textnormal{[...]} d'un degré commode}}, by which he likely means
...
\]
When $p=3$, there is however no ambiguity; assuming one does not wish to reduce the computatio
...
known to Schröder.
\end{quote}
It is quite possible that the irrational methods could be expressed using Schröder’s methods in one way or another, but while
...
\begin{quote}
% Ending the section and the page on a line-and-a-half indented quote following a
% half-line indented paragraph is particularly unsightly; turn the quote into
% a cul-de-lampe instead.
\shapepar{{0}
{0}b{0}\\
{0}t{-18.5}{37}\\
{13}e{0}}
\textfrench{%
Tout cela est fort abrégé; mais qui
nous délivrera des méthodes
abrégées, qui n'en
finissent
pas?}\end{quote}
...
ξ' = \tfrac{1}{2}q'+ \frac{1/\sqrt{12}}{q'}\sqrt{4yq'-{q'}^4}\text,\]
...
we have $\max_{y} \abs{ε_ξ'} \approx 0.00001048$, $\log_2 \max_{y} \abs{ε_ξ'} \approx -16.54$, one bit more
than with the rational method.
...
The idea for this optimization comes from \cite{Canon2018}, reproduced here with the author’s permission:
\begin{quotation}
 ...
 iterations.
\end{quotation}
Canon later elaborated on this REMOVE BEFORE FLIGHT citation goes here:
\begin{quotation}
...
and choose $Γ$, $κ$, $λ$, and $μ$ minimizing $\max_{y} \abs{ε_ξ'}$.

Unfortunately, computing $\max_{y} \abs{ε_ξ'}$ is not as nice as for the standard methods;
the introduction of $κ$, $λ$, and $μ$ breaks the monotonicity of $ε_ξ'$,
so that the local extrema of $ε_ξ'$ are not found in the same place as those of $ε_q'$.
Directly looking for zeros of the derivative of $ε_ξ'$ with respect to $f$ is impractical.
Instead we find the local maxima by numerical maximization on the four pieces whereon $q'$
is a smooth function of $f$.

That maximum can be minimized by a straightforward hill-climbing starting from
$Γ=\frac{1}{10}$, $κ=\frac{1}{2}$, $λ=\frac{1}{4}$, and $μ=3$;
