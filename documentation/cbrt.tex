...
We compute the faithfully rounded result $r_0$ as\[
r_0\DefineAs\round{x+Δ}\text,
\]
where the correction term $Δ$ is that of a high-order root finding method.
As usual, we call the infinite-precision correction term $Δ'$, and $r_0'\DefineAs x+Δ'$.
The truncation error is \[ε_{r_0}'\DefineAs\frac{\round{x+Δ'}}{\cuberoot y}-1\text.\]
The rounding error on the correction term is $δ\DefineAs\frac{Δ}{Δ'}-1$.
The overall error is thus\begin{align*}
ε_{r_0}\DefineAs&\frac{\round{x+Δ}}{\cuberoot y}-1\\
=&\pa{\frac{x+Δ'\pa{1+δ}}{\cuberoot y}}\pa{1+υ}-1\\
=&\pa{\frac{x+Δ'+\pa{x+Δ'-x}δ}{\cuberoot y}}\pa{1+υ}-1\\
=&\pa{1+ε_{r_0}'+\pa{ε_{r_0}'-ε_x}δ}\pa{1+υ}-1\text,
\end{align*}
where $\abs υ \le u$.
