...
\subsection*{Overview}
The general approach to compute a faithfully-rounded cube root of $y>0$ is the same as the one described in
\cite{KahanBindel2001}:
\begin{enumerate}
...
\]
so that, up to rounding errors\footnote{The ~.},\[
ε_ξ \approx ε_ξ' \DefineAs \frac{ξ'}{\sqrt[3] y}-1\text.
\]
...
Γ_{\mathrm{L}}&\DefineAs\argmin_{Γ\in\R}\max_{y}\abs{ε_ξ'}\\
...
However, evaluating it as\[
ξ = \tfrac{1}{2}q'+ \frac{1/\sqrt{12}}{q'}\sqrt{4yq'-{q'}^4}\]
yields performance comparable to the rational method.
