...
\subsection*{Finite difference formula}
In this section we prove a backward difference formula that makes it possible to compute an approximation of any derivative of a function on an equally-spaced grid at any desired order.
\begin{lemma}
Given a sufficient regular function $f\of{x}$ and two integers $m ≤ n$, there exist a family of rational numbers $\gl^{m}_{n,\gn}$, with $0 ≤ \gn ≤ n$ such that:
