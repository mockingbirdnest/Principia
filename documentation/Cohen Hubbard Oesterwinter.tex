...
&= \frac{1}{\pa{\gn - n} h} \pa{-n h \gd^{m}_{n - 1,\gn} - m \gd^{m - 1}_{n - 1,\gn}}
\end{align*}
...
\[
\gl^{m}_{n,\gn} = \frac{1}{\gn - n} \pa{-n \gl^{m}_{n - 1,\gn} - m \gl^{m - 1}_{n - 1,\gn}}
\]

and rewrite equation (\ref{fornbergbackwardpreliminary}) as:
\begin{equation}
...
Equation (\ref{fornbergbackward}) can be applied to the second derivative $f\dersecond\of{x_0}$ to yield an approximation which is still of order $n - m + 1$:
\[
...
\[
f\of{x_0 - h} = f\of{x_0} - h f\der\of{x_0} + h^2 \sum{m = 2}[n]{~\sum{\gn = 0}[n - 2]{\frac{\pa{-1}^m \gl^{m - 2}_{n - 2,\gn}}{\Factorial m} f\dersecond\of{x_0 - \gn h}}} + \BigO\of{h^{n + 1}}
\]

The summations are independent and can be exchanged:
\[
f\of{x_0 - h} = f\of{x_0} - h f\der\of{x_0} + h^2 \sum{\gn = 0}[n - 2]{\pa{\sum{m = 2}[n]{\frac{\pa{-1}^m \gl^{m - 2}_{n - 2,\gn}}{\Factorial m}}} f\dersecond\of{x_0 + \gn h}} + \BigO\of{h^{n + 1}} \\
\]

We can then define:
\[
\gh_{n, \gn} \DefineAs \sum{m = 2}[n]{\frac{\pa{-1}^m \gl^{m - 2}_{n - 2,\gn}}{\Factorial m}}
\]
