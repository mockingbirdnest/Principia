...
\subsection*{An optimization where the circle is outside of the wedge
$\plane KAB$}
...
$\bipoint KH$ in basis $\tuple{\bipoint KA, \bipoint KB}$, we
have
\[
{\bipoint HM} = \ga {\bipoint KA}\text,
\]
...
\scal{\bipoint HM}{\bipoint HM} = \frac{r^2} {\sin^2 \gq} =
                                  \frac{r^2} {1 - \cos^2 \gq}
\]
Eliminating $\bipoint HM$ between these two equations we obtain the following
conditions on $\ga$ for $\point H$ to be in the position shown in
figure~\ref{figOutside}:
\begin{align*}
\ga &\leq 0\\
...
$\plane KAB$ and tangent to $\straightline KA$.
