...
\end{itemize}
In this document we assume a base-2 floating-point number system with $M$ mantissa bits\footnote{In \texttt{binary64}, $M = 53$.} similar to the IEEE formats.  We define a real  function $\mant$ and an integer function $\expn$ denoting the mantissa and exponent of a real number, respectively:
\[
...
\frac{\Pi}{2} ≅ C + \gd C
\]
where $C$ is obtained by truncating $\frac{\Pi}{2}$ keeping only $\gk_1$ mantissa bits:
\[
C \DefineAs \floor{2^{-\gk_1} \mant\of{\frac{\Pi}{2}}} 2^{\expn\of{\frac{\Pi}{2}} + \gk_1} = \floor{2^{-\gk_1} \mant \of{\frac{\Pi}{2}}} 2^{\gk_1} \ULP\of{\frac{\Pi}{2}}
\]
and $\gd C$ is defined as $\round{\frac{\Pi}{2} - C}$.  Obviously we have:
\[
0 < \frac{\Pi}{2} - C < 2^{\gk_1} \ULP\of{\frac{\Pi}{2}}
\]
but if $\gk_1$ is chosen to cut the mantissa of $\frac{\Pi}{2}$ at a place where it has zeroes, we can actually have a stricter bound:
\[
...
\]
Since the $\ULP$ function is always a power of $2$ this implies:
\[
...
\]
In other words, we have a representation with a mantissa that has effectively $2 M - \gk_2$ bits and is such that multiplying $C$ by an integer less than or equal to $2^{\gk_2}$ is exact.  The representation of $\frac{\Pi}{2}$ has three zeroes after the 18th bit of its mantissa, so by taking $\gk_1 = 18$ we have $\gk_2 = 14$.

\subsection*{Argument Reduction for Small Angles}
The reduction proceeds as follows:
\begin{itemize}
\item if $\abs x < \frac{\Pi}{4}$ then $\hat x = x$ and $\gd \hat x = 0$;
\item if $\abs x < \frac{\Pi}{2}_H$ then \\
$n \DefineAs \round{x \frac{2}{\Pi}}$ \\
$\pa{\hat x, \gd \hat x} = \round{x - n \frac{\Pi}{2}_H} - \round{n \frac{\Pi}{2}_L}$; 
\end{itemize}
