...
The \textit{Mathematica} function \texttt{GeneralMiniMaxApproximation} produces a minimax polynomial $p\of{x}$ approximating a function $f\of{x}$ by minizing the quantity $\frac{f\of{x} - p\of{x}}{g\of{x}}$.  By choosing $g\of{x}$ appropriately, we can obtain an approximation that minimizes either the absolute or relative error on the result.
