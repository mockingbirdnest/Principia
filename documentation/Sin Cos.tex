...
\DeclareMathOperator{\latticereduce}{\StandardSymbol{LatticeReduce}}
\DeclareMathOperator{\integerroots}{\StandardSymbol{IntegerRoots}}
...
\KwData{Two functions $F_1$ and $F_2$, and two positive integers $M$, $T$.}
\KwResult{All $t \in \intclos{-T}{T}$ such that $\abs{F_i\of{t} \cmod 1} < \frac{1}{M}$ for $i \in \set{1, 2}$.}
...
To look for accurate table values, we need to search around a point different from zero.  Furthermore, if $T$ is small enough, we are better off doing an exhaustive search than going through the fairly expensive lattice reduction.  For an exhaustive search, we let the caller specify whether the search should proceed by increasing or decreasing values to make sure that we find, to the extent possible, the solution closest to the centre of $\mathscr{I}_k$.  Algorithm \ref{ISBCS} spells out the processing of one interval specifically for the $\sin$ and $\cos$ functions.
...
\SetNlSty{texttt}{}{.}
\SetAlgoNlRelativeSize{0}
\KwData{The midpoint of the interval to search, $x_{\text{mid}}$, a search direction $\texttt{UP}$ or $\texttt{DOWN}$, and two positive integers $M$, $T$.}
\KwResult{A $t \in \intclos{-T}{T}$ such that $x_{\text{mid}} + \frac{t}{N}$ is an accurate $x$-value for $\sin$ and $\cos$; furthermore, if the search direction is $\texttt{UP}$, $t$ is as close as possible to $-T$ and the search direction is $\texttt{DOWN}$, $t$ is as close as possible to $T$.}
...
\SetNlSty{texttt}{}{.}
\SetAlgoNlRelativeSize{0}
\KwData{$\gD$, the half-size of intervals, and $k$, the index of the interval to search.}
\SetKwFunction{SimultaneousBadCaseSearch}{SimultaneousBadCaseSearch}
$T_0$ \leftarrow = $\sqrt[3]{M N}$\;
$x_h$ \leftarrow $2 k \gD + T_0$\;
$x_l$ \leftarrow $2 k \gD - T_0$\;
\Repeat{a solution is found}{
  $T$ \leftarrow $T_0$\;
  \Repeat{}{
    \tcc{Search in the interval $\intclos{x_h - T}{x_h + T}$.}
    Let $F_1 = \FunctionBody{t}{N \sin\of{x_h + \frac{t}{N}}}$ and $F_2 = \FunctionBody{t}{N \cos\of{x_h + \frac{t}{N}}}.$\;
    $y_h$ \leftarrow \SimultaneousBadCaseSearch{$F_1$, $F_2$, $M$, $T$}.\;
    \uIf{$y_h$ = \texttt{FAIL}}{
      $x_h$ \leftarrow $x_h - T / 2$.\;
      $T$ \leftarrow $T / 2$.\;
    }
    \Else{
    }
...
\caption{IntervalSimultaneousBadCaseSearch2.}
\end{algorithm}



\footnotetext{The notation $\iround{C · P_i\of{T \gt}}$ means that we round to the nearest each coefficient of $C · P_i\of{T \gt}$.  This gives an element of $\Integers [ \gt ]$} 

\marginnote{TODO(phl): Document the tables and all the tricks that went into their generation.}

\subsection*{Nguyễn and Stehlé's L2 algorithm}
...
In practice we chose above $\gD = 2^{-10}$\marginnote{TODO(phl): Cross-reference the accurate tables section.} and the accurate tables construction yielded $h_{\text{max}} < \gD + 2^{-17.834}$.  The minimax computation results in a 1-degree polynomial $p_s$ with with a residual smaller than $2^{-85.534}$ and a 1-degree polynomial $p_c$ with a residual smaller than $2^{-51.466}$ (before rounding the coefficients to machine numbers).
