...
\subsubsection*{Cody-Waite Argument Reduction Using the Two-Term Approximation}
...
\subsubsection*{Cody-Waite Argument Reduction Using the Three-Term Approximation}
...
On machines that have an FMA instruction, we use the Boldo-Daumas-Li reduction algorithm as described in \cite{BoldoDaumasLi2009} and \cite[204-206]{Muller2016}.  Using our notation, this algorithm proceeds as follows:
\[
\begin{dcases}
...
\end{dcases}
\]
where $R$, $C_1$, and $C_2$ are carefully constructed constants based on $C$, the modulo of the reduction.  In our case $C = \frac{\Pi}{2}$ and \cite[1141]{BoldoDaumasLi2009} prove (theorem 3) that $\abs{R \; x - n} \leq 1/2$.  Since $R = \round{\frac{2}{\Pi}}$, we obtain $n = \iround{\round{\frac{2}{\Pi}} \; x}$ which is similar to the definition we used in the Cody-Waite reduction above.
