...
\subsection*{Sin Between Accurate Table Entries}
Let $\pa{x_k, s_k, c_k}$ be an accurate table entry.  $x_k$ is close to $2 k \gD$ and the accurate table interval containing $x_k$ is $\intclos{\pa{2 k - 1}\gD}{\pa{2 k + 1}\gD}$\marginnote{TODO(phl): This cannot be a closed interval.}.  The algorithm starts by choosing, from the argument $x$, a $k$ such that $x \in \intclos{\pa{2 k - 1}\gD}{\pa{2 k + 1}\gD}$ and computing $h = x - x_k$.  We are therefore looking for an approximation of the form:
\[
...
It is difficult to find a theoretical justification for the error function used in that approximation, as the polynomials for $\sin$ and $\cos$ both contribute in complicated ways to the final error.  In practice it appears that an absolute error bound results in a slightly lower error on the result, so we call \texttt{GeneralMiniMaxApproximation} with:
\[
\begin{dcases}
h\of{x} &\DefineAs x^2 \\
f\of{x} &\DefineAs \frac{\sin x - x}{x^3} \\
...
In practice we chose above $\gD = 2^{-10}$\marginnote{TODO(phl): xref} and the accurate tables construction yielded $h_{max} < \gD + 2^{-17.834}$.  The minimax computation results in a 1-degree polynomial $p_s$ with an absolute error smaller than $2^{-85.746}$ before rounding the coefficients to machine numbers.
