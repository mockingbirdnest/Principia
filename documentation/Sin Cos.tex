...
It is difficult to find a theoretical justification for the error function used in these approximations, as the polynomials $p_s$ and $p_c$ contribute in complicated ways to the final error on $\sin$ and $\cos$.  In practice it appears that an absolute error bound results in a slightly lower final relative error, so for $\sin$ we call \texttt{GeneralMiniMaxApproximation} with:\marginnote{TODO(phl): Use the funky error.}
\[
\begin{dcases}
h\of{x} &\DefineAs x^2 \\
f\of{x} &\DefineAs \frac{\sin x - x}{x^3} \\
...
\subsection*{Around Table Entries}
If $\red x \in \intclos{\pa{2 k - 1}\gD}{\pa{2 k + 1}\gD}$ we compute $h = \red x - x_k$.  It is essential that this subtraction be exact, and therefore that the conditions of Sterbenz's lemma be met: we must have $\red x / 2 \le x_k \le 2 \red x$.  Based on the range of $\red x$, a sufficient condition for this is:
\[
\frac{2 k + 1}{2} \gD \le x_k \le 2 \pa{2 k - 1} \gD
\]
Because $x_k$ is close to $2 k \gD$ the left part of this condition is trivially met for all $k > 0$.  However the second part is only trivially met if $k > 1$.  If $k = 1$ it becomes $x_k \le 2 \gD$.  In other words, when building the accurate tables we must only look for $x_1$ \emph{below} $2 \gD$.\marginnote{TODO(phl): xref in the accurate tables section.  Also say something about interval 0.}

For $\sin$ we do not use the interval with $k = 0$  (see \nameref{secsinnearzero}, above) but for $\cos$ we do.  That interval is special because it has bounds $\intclos{0}{\gD}$ and $x_0 = 0$.  The computation of $h$ is therefore trivially correct.

The relative errors on the minimax polynomials may be rewritten as:
\begin{align*}
p_s\of{x^2} &= \pa{1 + \gz_1} \frac{\sin x - x}{x^3} \qquad & \abs{\gz_1} < 2^{-52.689} \\
p_c\of{x^2} &= \pa{1 + \gz_2} \frac{\cos x - 1}{x^2} \qquad & \abs{\gz_2} < 2^{-51.466}
\end{align*}
...
t_5 &= \round{s_k} \; t_2 \; t_3 \pa{1 + \gd_5} \pa{1 + \gd_6} \\
t_6 &= t_1 \; t_4 \pa{1 + \gd_7} \\
