...
\[
\abs{\frac{\Pi}{2} - C_1 - C_2} \leq 2^{-M + 3} \ULP\of{C_1} \leq 2^{-2 M + 4}
\]
...
\todo{Include $\gd \red x$ in this analysis.}
\[
...
where:
\[
C_3 = \round{\frac{\Pi}{2} - C_1 - C_2} = \pa{\frac{\Pi}{2} - C_1 - C_2} \pa{1 + \gd_1}
\]

Let $\gz$ be the relative error introduced by $\dwminusfp$.  Theorem 2.2 of \cite{MullerRideau2022} indicates that $\abs{\gz} < 2^{1 - 2 M}$.  The result of the above computation is:
\[
...
\begin{align*}
\abs{\frac{\Pi}{2} - \pa{C_1 + C_2 + C_3}} &\leq \gd_1 \; 2^{-2 M + 4} = 2^{-3 M + 4} \\
\gd_1 \; C_3 &= \gd_1 \pa{1 + \gd_1} \pa{\frac{\Pi}{2} - C_1 - C_2} \leq \gd_1 \pa{1 + \gd_1} 2^{-2 M + 4} \\
\gz \pa{1 + \gd_1} C_3 &\leq \gz \pa{1 + \gd_1}^2 \; 2^{-2 M + 4} \\
\gz \pa{x - n \pa{C_1 + C_2}} &= \gz \pa{\red x + \gd \red x}
\end{align*}
