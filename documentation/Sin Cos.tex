...
In this scheme we use a two-term approximation $C_1 + C_2$ of the modulus.  Using our notation, the algorithm proceeds as follows:
\[
\begin{dcases}
...
\end{dcases}
\]
where $R$, $C_1$, and $C_2$ are carefully constructed constants based on $C$, the modulo of the reduction\footnote{These constants are distinct from those used for Cody-Waite reduction.}.  In our case $C = \frac{\Pi}{2}$ and \cite[1141]{BoldoDaumasLi2009} prove (theorem 3) that $\abs{R \; x - n} \leq 1/2$.  Since $R = \round{\frac{2}{\Pi}}$, this leads to $n = \iround{\round{\frac{2}{\Pi}} \; x}$, a definition similar to the one we used in the Cody-Waite reduction above.
...
\end{equation}
where we have used the fact that $C_1$ is very close to $\frac{\Pi}{2}$ and thus that $\ULP\of{C_1} = 2^{-M + 1}$.  Furthermore, the absolute error on the reduction is such that:
\[
...
\]
and the absolute error on the reduction is the difference between this quantity and $x - n \frac{\Pi}{2}$:
\begin{align*}
...
\abs{\gz_1 \; \pa{x - n \pa{C_1 + C_2}}} &= \gz_1 \; \abs{\red x + \gd \red x} = \gz_1 \; \abs{\red x} \pa{1 + \gd_2} \leq \abs{n} \pa{1 + \gd_2} 2^{\gk_3 - 3 M + 5} \\
...
\begin{align*}
\abs{\mathscr{E}} &\leq \abs{n} \; \pa{\pa{\gd_1 \pa{2 + \gd_1} + \pa{1 + \gd_1}^2 2^{-2 M + 1} + \gd_2 \; 2^{\gk_3 - M + 1}} \; 2^{-2 M + 4} + 2^{\gk_3 - 3 M + 5}} \\
&< \abs{n} \; \pa{3 \times 2^{-3 M + 4} + 2^{\gk_3 - 3 M + 5}} = \abs{n} \; 2^{-3 M + 4} \pa{3 + 2^{\gk_3 + 1}}
\end{align*}
where we have used:
\[
\gd_1 \pa{2 + \gd_1} + \pa{1 + \gd_1}^2 2^{-2 M + 1} + \gd_2 \; 2^{\gk_3 - M + 1} < 3 \times 2^{-M}
\]
