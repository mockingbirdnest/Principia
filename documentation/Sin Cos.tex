...
If $x \in \intclos{\pa{2 k - 1}\gD}{\pa{2 k + 1}\gD}$ we first compute $h = x - x_k$.  It is essential that this subtraction be exact, and therefore that the conditions of Sterbenz's lemma be met: we must have $x / 2 \le x_k \le 2 x$.  Based on the range of $x$, a sufficient condition for this is:
\[
\frac{2 k + 1}{2} \gD \le x_k \le 2 \pa{2 k - 1} \gD
\]
Because $x_k$ is close to $2 k \gD$ the left part of this condition is trivially met for all $k > 0$.  However the second part is only trivially met if $k > 1$.  If $k = 1$ it becomes $x_k \le 2 \gD$.  In other words, when building the accurate tables we must only look for $x_1$ \emph{below} $2 \gD$.\marginnote{TODO(phl): xref in the accurate tables section.}
...
The remaining steps of the computation are then as follows:\marginnote{TODO(phl): Align the notation in the notebook.}
\[
\begin{dcases}
...
\end{dcases}
\]
where we have systematically made the rounding of the accurate table elements $s_k$ and $c_k$ explicit.
