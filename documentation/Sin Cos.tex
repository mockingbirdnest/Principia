...
\end{dcases}
\]
The products $n \; C_2$ and $n \; C'_2$ are exact thanks to the $\gk_2$ trailing zeroes of $C_2$ and $C'_2$.  The subtraction $x - n \; C_2$ is exact by Sterbenz's Lemma.  $QuickTwoSum$ performs an exact addition using algorithm 3 of \cite{HidaLiBailey2007}; is usable in this case because clearly $\abs{\gd z} < \abs z$.
$LongSub$ is the obvious adaptation of the algorithm $LongAdd$ presented in section 5 of \cite{Linnainmaa1981}, which implements very precise (but not exact) double-precision arithmetic.
...
from which we deduce an upper bound on the absolute error of the reduction:
\begin{align*}
\abs{y - y' - \gd y - \pa{x - n \frac{\Pi}{2}}} &\leq  \\
\end{align*}
