...
\pa{u, \gd u} &= TwoDifference\of{y, z} \\
\pa{\hat x, \gd \hat x} &= \pa{u, \gd u - \gd z}
\end{dcases}
\]
The products $n \; C_2$ and $n \; C'_2$ are exact thanks to the $\gk_2$ trailing zeroes of $C_2$ and $C'_2$.  The subtraction $x - n \; C_2$ is exact by Sterbenz's Lemma.  $QuickTwoSum$ performs an exact addition using algorithm 3 of \cite{HidaLiBailey2007}; it is usable because clearly $\abs{\gd y} < \abs{y'}$.
$TwoDifference$ performs an exact subtraction using algorithm 4 of \cite{HidaLiBailey2007}.  Note that the last three operations in the sequence are effectively an implementation of the middle version of $ThreeSum$ in figure 7 of \cite{HidaLiBailey2007}
...
\]
Let $\gz_2$ be the relative error introduced by $ThreeSum$.  Lemma 11 of \cite{HidaLiBailey2007} indicates that $\abs{\gz_2} < 2^{-2 M}$.  The error computation proceeds as follows:
\begin{align*}
...
& \qquad \leq 2^{\gk_2 + \gk'_2 + \gk''_2} \pa{2^{-2 M - 1} + 2^{-2 M} + 2^{-3 M - 1}} \pa{1 + 2^{-2 M}} \ULP\of{\frac{\Pi}{2}} + 2^{-2 M} \frac{\Pi}{4} \\
& \qquad = 2^{\gk_2 + \gk'_2 + \gk''_2 - 2 M} \pa{\frac{3}{2} + 2^{-M - 1}} \pa{1 + 2^{-2 M}} \ULP\of{\frac{\Pi}{2}} + 2^{-2 M} \frac{\Pi}{4} \\
& \qquad < 2^{\gk_2 + \gk'_2 + \gk''_2 - 2 M + 1} \ULP\of{\frac{\Pi}{2}} + 2^{-2 M} \frac{\Pi}{4}
\end{align*}
A sufficient condition for the reduction to be correctly rounded with $\gk_3$ extra bits of accuracy is for this error to be less than $2^{-\gk_3 - 1} \abs{\ULP\of{\hat x}}$ which itself is less than $2^{-\gk_3 - M} \abs{\hat{x}}$.  Therefore we want\footnote{Note that this bound is not particularly strict, because when $\abs{\hat x}$ is small, the upper bound $\frac{\Pi}{4}$ on the second term is very pessimistic, and it dominates the right hand side.}:
\begin{align*}
\abs{\hat x} &\geq 2^{\gk_3 - M}\pa{2^{\gk_2 + \gk'_2 + \gk''_2 + 1} \ULP\of{\frac{\Pi}{2}} + \frac{\Pi}{4}} \\
&= 2^{\gk_3 - M}\pa{2^{\gk_2 + \gk'_2 + \gk''_2 - M + 2}  + \frac{\Pi}{4}}
\end{align*}
and it is therefore sufficient to have:
\[
\abs{\hat x} \geq 2^{\gk_3 - M}\pa{2^{\gk_2 + \gk'_2 + \gk''_2 - M + 2}  + 1}
\]
If we choose $\gk_3 = 18$ as above, and $\gk_2 = 18$ we find that $\gk'_2 = 13$ and $\gk''_2 = 14$.  Therefore, the desired accuracy is obtained as long as $\abs{\hat x} \geq 65 \times 2^{-41} ≃ 3.0 \times 10^{-11}$.
