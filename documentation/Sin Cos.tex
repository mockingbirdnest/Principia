...
\DeclareMathOperator{\dwminusfp}{\StandardSymbol{DWMinusFP}}
...
\begin{itemize}[nosep]
\item perform argument reduction using the Cody--Waite or Boldo--Daumas--Li algorithm in double precision (see \cite[379]{MullerBrisebarreDeDinechinJeannerodLefevreMelquiondRevolStehleTorres2010}, \cite[204]{Muller2016});
\item if the argument is too large, or if argument reduction loses too many bits (\idest, the argument is close to a multiple of $\frac{\Pi}{2}$), fall back to Payne--Hanek reduction;
\item use accurate tables and polynomial approximations to compute \texttt{Sin} or \texttt{Cos} with extra accuracy;
...
\[
\begin{dcases}
\red x + \gd \red x \cong x \pmod{\frac{\Pi}{2}} \\
...
\subsection*{Cody--Waite Argument Reduction}

On machines that do not have a Fused Multiply-Add (FMA) instruction, we use the Cody--Waite reduction algorithm as described in \cite[379-381]{MullerBrisebarreDeDinechinJeannerodLefevreMelquiondRevolStehleTorres2010}.  This algorithm requires that we build approximations of $\frac{\Pi}{2}$ with extra accuracy and analyse the circumstances under which they may be used and the errors that they entail on the reduced argument.
...
\subsubsection*{Two-Term Approximation of $\frac{\Pi}{2}$}
...
Noting that the absolute error on the rounding that appears in the definition of $\gd C_1$ is bounded by $\frac{1}{2} \ULP\of{\frac{\Pi}{2} - C_1}$, we obtain the absolute error on the two-term approximation\footnote{With the chosen value $\gk_1 = 8$, the two sides of this inequality turn out to be $2^{-103.217}$ and $2^{-101}$.  The difference comes from the fact that $\abs{\frac{\Pi}{2} - C_1 - \gd C_1}$ has three zeroes after the last bit of its significand, but the theoretical computation above assumes the worst, \idest, a run of ones.}:
\begin{align}
...
\subsubsection*{Three-Term Approximation of $\frac{\Pi}{2}$}
...
\subsubsection*{Cody--Waite Argument Reduction Using a Two-Term Approximation}
...
\subsubsection*{Cody--Waite Argument Reduction Using a Three-Term Approximation}
...
\]
Let $\gz_2$ be the relative error introduced by $\dwplusfp$.  Theorem 2.2 of \cite{MullerRideau2022} indicates that $\abs{\gz_2} \leq 2^{1 - 2 M}$.  The error computation proceeds as follows:
\begin{align*}
...
\subsection*{Boldo--Daumas--Li Argument Reduction}

On machines that have an FMA instruction, we use the Boldo--Daumas--Li reduction algorithm as described in \cite{BoldoDaumasLi2009} and \cite[204-206]{Muller2016}.  

\subsubsection*{Boldo--Daumas--Li Argument Reduction Using a Two-Term Approximation}

In this scheme we use a two-term approximation $C_1 + C_2$ of the modulus $C$.  Let $R \DefineAs \round{\frac{1}{C}}$.  Using our notation, the algorithm proceeds as follows:
\[
\begin{dcases}
n &= \round{\round{3 \cdot 2^{M - 2} + R \; x} - 3 \cdot 2^{M - 2}} \\
u &= \round{x - n \; C_1} \\
\red x &= \round{u - n \; C_2} \\
\gr_h &= \round{n \; C_2} \\
\gr_l &= \round{n \; C_2 - \gr_h} \\
\pa{t, \gd t} &= \quicktwodifference\of{u, \gr_h} \\
\gd \red x &= \round{\round{\round{t - \red x} + \gd t} - \gr_l}
\end{dcases}
\]
where $C_1$, and $C_2$ are carefully constructed constants\footnote{These constants are distinct from those used for Cody--Waite reduction.} based on $C$.  In our case $C = \frac{\Pi}{2}$ and $R = \round{\frac{2}{\Pi}}$. \cite[1141]{BoldoDaumasLi2009} prove (theorem 3) that $\abs{R \; x - n} \leq 1/2$.  This leads to $n = \iround{\round{\frac{2}{\Pi}} \; x}$, a definition similar to the one we used in the Cody--Waite reduction above.

The key result of \cite{BoldoDaumasLi2009} is that:
\[
\red x + \gd \red x = x - n \pa{C_1 + C_2}
\]
\cite[205]{Muller2016} notes that:
\begin{equation}
\abs{\frac{\Pi}{2} - \pa{C_1 + C_2}} \leq 2^{-M + 3} \ULP\of{C_1} \leq 2^{-2 M + 4}
\label{eqnmullermodulo}
\end{equation}
where we have used the fact that $C_1$ is very close to $\frac{\Pi}{2}$ and thus that $\ULP\of{C_1} = 2^{-M + 1}$.  Furthermore, the absolute error $\mathscr E$ on the reduction is such that:
\[
\abs{\mathscr{E}} \leq \abs{n} \; 2^{-M + 3} \ULP\of{C_1} = \abs{n} 2^{-2 M + 4}
\]

The two-term reduction guarantees $\gk_3$ extra bits of accuracy if this error is less than $2^{-\gk_3 - 1} \ULP\of{\red x}$:

\[
\abs{n} \; 2^{-2 M + 4} \leq 2^{-\gk_3 - 1} \ULP\of{\red x} \leq 2^{-\gk_3 - M} \abs{\red x}
\]
A sufficient condition for this is\footnote{Note that the error bound is fundamentally different from the one we obtained for the Cody--Waite reduction.  For Cody--Waite we have to choose upfront an upper bound on $\abs{n}$ (and therefore $\abs{x}$) based on the number of zeroes at the end of the significand of $C_1$ (resp. $C_2$).  This is necessary so that the product $n \; C_1$ (resp. $n \; C_2$) be exact.  Here we don't need an \emph{a priori} upper bound because \cite{BoldoDaumasLi2009} always yields an exact value.  So we can apply the algorithm to any argument and check \emph{post hoc} if the reduced argument is large enough.  Most of the time it will be, unless the input argument is extremely large or extremely close to a multiple of $\frac{\Pi}{2}$.  This means that the Boldo--Daumas--Li reduction is usable over a much larger range that the Cody--Waite reduction.}:
\begin{equation}
\abs{\red x} \geq \abs{n} \; 2^{\gk_3 - M + 4}
\label{eqnboldo2}
\end{equation}
If $\gk_3 = 18$ the numeric value of the multiplier $2^{\gk_3 - M + 4}$ is $2^{-31} \simeq 4.7 \times 10^{-10}$.

\subsubsection*{Boldo--Daumas--Li Argument Reduction Using a Three-Term Approximation}

If the condition (\ref{eqnboldo2}) is not met, we extend the algorithm as suggested by \cite[1139]{BoldoDaumasLi2009} and \cite[205]{Muller2016}:
\[
\begin{dcases}
w &= \round{n \; C_3} \\
\pa{\red x, \gd \red x} &= \dwminusfp\of{\pa{\red x, \gd \red x}, w}
\end{dcases}
\]
where:
\[
C_3 = \round{\frac{\Pi}{2} - \pa{C_1 + C_2}} = \pa{\frac{\Pi}{2} - \pa{C_1 + C_2}} \pa{1 + \gd_1}
\]

Let $\gz_1$ be the relative error introduced by $\dwminusfp$.  Theorem 2.2 of \cite{MullerRideau2022} indicates that $\abs{\gz_1} \leq 2^{1 - 2 M}$.  The result of the above computation is:
\[
\dwminusfp\of{\pa{\red x, \gd \red x}, w} = \pa{1 + \gz_1}\pa{x - n \pa{C_1 + C_2} - \pa{1 + \gd_2} \; n \; C_3}
\]
and the absolute error $\mathscr{E}$ on the reduction is the difference between this quantity and $x - n \frac{\Pi}{2}$:
\begin{align*}
\mathscr{E} &= \pa{1 + \gz_1}\pa{x - n \pa{C_1 + C_2} - \pa{1 + \gd_2} \; n \; C_3} - \pa{x - n \frac{\Pi}{2}} \\
&= n \pa{\frac{\Pi}{2} - \pa{C_1 + C_2 + C_3} - \gd_2 \; C_3} + \gz_1 \; \pa{x - n \pa{C_1 + C_2}} - \gz_1 \; \pa{1 + \gd_2} \; n \; C_3
\end{align*}
The first term may be simplified by noting that:
\begin{align*}
\frac{\Pi}{2} - \pa{C_1 + C_2 + C_3} &= -\gd_1 \pa{\frac{\Pi}{2} - \pa{C_1 + C_2}} \\
\gd_2 \; C_3 &= \gd_2 \pa{1 + \gd_1} \pa{\frac{\Pi}{2} - \pa{C_1 + C_2}}
\end{align*}
and therefore:
\[
\frac{\Pi}{2} - \pa{C_1 + C_2 + C_3} - \gd_2 \; C_3 = -\pa{\gd_1 + \gd_1 \gd_2 + \gd_2} \pa{\frac{\Pi}{2} - \pa{C_1 + C_2}}
\]

From this we deduce the following bounds for the terms of $\mathscr{E}$, using (\ref{eqnmullermodulo}) and the fact that, when we use a three-term approximation, (\ref{eqnboldo2}) does not hold:
\begin{align*}
\abs{\frac{\Pi}{2} - \pa{C_1 + C_2 + C_3} - \gd_2 \; C_3} &\leq \pa{\gd_1 + \gd_1 \gd_2 + \gd_2} \; 2^{-2 M + 4} \\
\abs{\gz_1 \; \pa{x - n \pa{C_1 + C_2}}} &= \gz_1 \; \abs{\red x + \gd \red x} = \gz_1 \; \abs{\red x} \pa{1 + \gd_3} \leq \abs{n} \pa{1 + \gd_3} 2^{\gk_3 - 3 M + 5} \\
\abs{\gz_1 \; \pa{1 + \gd_2} \; C_3} &\leq \gz_1 \; \pa{1 + \gd_1}\pa{1 + \gd_2} \; 2^{-2 M + 4} \leq \pa{1 + \gd_1}\pa{1 + \gd_2} \; 2^{-4 M + 5}
\end{align*}

Putting all these inequalities together we obtain the following bound for the absolute error:
\begin{align*}
\abs{\mathscr{E}} &\leq \abs{n} \; \pa{\pa{\pa{\gd_1 + \gd_1 \gd_2 + \gd_2} + \pa{1 + \gd_1}\pa{1 + \gd_2} 2^{-2 M + 1} + \gd_3 \; 2^{\gk_3 - M + 1}} \; 2^{-2 M + 4} + 2^{\gk_3 - 3 M + 5}} \\
&< \abs{n} \; \pa{3 \times 2^{-3 M + 4} + 2^{\gk_3 - 3 M + 5}} = \abs{n} \; 2^{-3 M + 4} \pa{3 + 2^{\gk_3 + 1}}
\end{align*}
where we have used:
\[
\pa{\gd_1 + \gd_1 \gd_2 + \gd_2} + \pa{1 + \gd_1}\pa{1 + \gd_2} 2^{-2 M + 1} + \gd_3 \; 2^{\gk_3 - M + 1} < 3 \times 2^{-M}
\]

As before, the three-term reduction guarantees $\gk_3$ extra bits of accuracy if:
\[
\abs{\mathscr{E}} \leq 2^{-\gk_3 - 1} \ULP\of{\red x} \leq 2^{-\gk_3 - M} \abs{\red x}
\]
which is satisfied if:
\begin{equation}
\abs{\red x} \geq \abs{n} \; 2^{\gk_3 - 2 M + 4} \pa{3 + 2^{\gk_3 + 1}}
\label{eqnboldo3}
\end{equation}
If $\gk_3 = 18$ the numeric value of the multiplier is $524291 \times 2^{-84} \simeq 2.7 \times 10^{-20}$.

\subsubsection*{Applicability of the Boldo--Daumas--Li Argument Reduction}

If $\abs{n}$ is too large (that is, if $\abs{x}$ is too large) the right-hand side of (\ref{eqnboldo2}) and (\ref{eqnboldo3}) becomes so large that no reduced angle $\red x$ is acceptable: this happens if the right-hand side is larger than $\frac{\Pi}{4}$.  When deciding for which angles it is worthwhile to attempt the Boldo--Daumas--Li reduction, it is important to note that all the work done for the two-term reduction is reused by the three-term reduction, since the latter just performs more operations on the result obtained by the former.  This differs from the Cody--Waite reduction, where the result of the two-term reduction is not usable by the three-term reduction.  Therefore, there is not much harm in trying the two-term Boldo--Daumas--Li reduction if there is a good probability that, if it fails, the three-term reduction will succeed.

In our implementation we choose to perform the Boldo--Daumas--Li reduction if there is a possibility that it succeeds with two terms according to (\ref{eqnboldo2}), that is if:
\[
\abs{n} < \frac{\Pi}{4} \; 2^{M - \gk_3 - 4}
\]
or equivalently if:
\[
\abs{x} < \Pi^2 \; 2^{M - \gk_3 - 7}
\]

The numerical value of this threshold is approximately $2.6 \times 10^9$, large enough for practical purposes.

For the largest value of $\abs{n}$ the right-hand side of (\ref{eqnboldo3}) is:
\[
\Pi \; 2^{-M - 2} \; \pa{3 + 2^{\gk_3 + 1}}
\]
whose numerical value is approximately $4.6 \times 10^{-11}$.  There is therefore a high probability that, even if the two-term reduction fails, the three-term reduction will succeed and the work done by the two-term reduction will not be wasted.

\subsection*{Fallback}

If any of the conditions above is not met, we fall back to the Payne--Hanek argument reduction algorithm as described in \cite[154-157]{Muller1997}.

\section*{Accurate Tables and Their Generation}

First, a remark on the notation.  This section (as well is the code in \linebreak\texttt{accurate\_table\_generator*.hpp}) follows the notation of \cite{StehléZimmermann2005}.  In particular, $N$ is related to the number of bits in the significand of the floating-point type, and $M$ is related to the number of zeroes or ones after the significand of the accurate values.  Specifically, since we use \texttt{binary64} and want 20 zeroes or ones after the significand (to minimize the probability of expensive fall backs to the CORE-MATH implementation), we have $N = 2^{53}$ and $M = 2^{20}$. 

The accurate tables are made of triples $\tuple{x_k, s_k, c_k}$ where $x_k$ is chosen such that $\sin x_k$ and $\cos x_k$ are very close to machine numbers, \idest, have a string of zeroes or ones after the last bit of their significand.  The tabulated value $s_k$ (respectively, $c_k$) is then obtained by rounding $\sin x_k$ (respectively, $\cos x_k$) to the nearest machine number.  See \nameref{overview} below for the details of how these tables are used for implementing $\sin$ and $\cos$.
...
The following table summarizes the impact of each of the optimizations described above.  They correspond to a search of 18 zeroes or ones after the significand.  The ``Baseline'' row is the time with all optimizations enabled.  Each of the other rows gives the timing with exactly one optimization disabled:
\begin{center}
\begin{tabular}{lrr}
...
In practice we choose $\gD' = 2^{-10} \pa{1 + \frac{7}{32}}$ (see \nameref{cutoffchoice}), and compute a degree-1 polynomial with a residual smaller than $2^{-74.142}$ (before rounding the coefficients to machine numbers).
...
\item The relative error of the polynomial approximation of $\sin$ near zero grows like $\BigO\of{{\gD'}^2}$.  This is because the dominant term of the error is due to the computation of ${\red x}^3$ and to the rounding of the degree-0 coefficient of $p_{s0}$\footnote{With the notation of \nameref{secsinnearzero}, the dominant term of the relative error on the result is $\pa{\gd_1 + \gd_2 + \gd_3 + \gd_4 + \gz_2} \frac{\sin \red x - \red x}{\sin \red x}$.}.  A smaller value of $\gD'$ minimizes this error.
\item The relative error of the accurate tables algorithm for $\sin$ on $\mathscr{I}_1$ is fairly large near $\gD$.  This is because any error on the accurate value $s_1$ is amplified by the  division by $\sin \red x$ when $\red x$ is small\footnote{With the notation of \nameref{secerroranalysissin}, the dominant term of the relative error is $\gz_2 \frac{-\round{s_k} + \round{c_k} \cos\of{\red x - x_k}}{\sin \red x}$.}.  A larger value of $\gD'$ minimizes this error.
\end{itemize}
...
\]
The function $r$ can be computed using \textit{Mathematica} interval arithmetic over the triangular domain $\abs{\red x} \le \gD$, $\abs{\gd \red x} \le 2^{-M} \abs{\red x}$.  Plotting shows\todo{See if this can be proven rigourously.} that this function reaches its extrema at the corners of the domain, which is logical because we expect the minimax polynomial to reach its largest errors on the bounds of the optimization interval.  In practice we find that $\abs{r\of{\red x, \gd \red x}} < 2^{-70.281}$ (as expected, this is a bit worse than the error originating from the angle reduction).  The rounding test must be done with $e = \texttt{0x1.0000'D28F'8E40'4p0}.$\todo{$\gk_3 = 19$ would gain 0.7 bits.  Worthwhile?}
...
The first step of the computation is to evaluate $s_k + h c_k$ with extra accuracy.  See \nameref{firstorder} for the analysis of this computation:
\[
...
t_3 &\DefineAs \round{h + \pa{\gd \red x + \gd \red x}} \\
t_4 &\DefineAs \round{\round{h^2} \; h} \\
t_5 &\DefineAs \round{\round{\round{\round{s_k} \; h } \; t_3} \; t_2} \\
...
t_3 &= \pa{h + 2 \; \gd \red x} \pa{1 + \gd_1} \\
t_4 &= h^3 \pa{1 + \gd_3} \pa{1 + \gd_4} \\
t_5 &= \round{s_k} \; h \; t_3 \; t_2 \; \pa{1 + \gd_2} \pa{1 + \gd_5} \pa{1 + \gd_6} \\
...
The first step of the computation is to evaluate $c_k - s_k h$ with extra accuracy.  See \nameref{firstorder} for the analysis of this computation:
\[
...
t_3 &\DefineAs \round{h + \pa{\gd \red x + \gd \red x}} \\
t_4 &\DefineAs \round{\round{h^2} \; h} \\
t_5 &\DefineAs \round{\round{\round{\round{c_k} \; h} \; t_3} \; t_2} \\
t_6 &\DefineAs \round{\round{t_4 \; t_1} + \gd \red x} \\
...
t_3 &= \pa{h + 2 \; \gd \red x} \pa{1 + \gd_1} \\
t_4 &= h^3 \pa{1 + \gd_3} \pa{1 + \gd_4} \\
t_5 &= \round{c_k} \; h \; t_3 \; t_2 \; \pa{1 + \gd_2} \pa{1 + \gd_5} \pa{1 + \gd_6} \\
t_6 &= \pa{t_4 \; t_1 \; \pa{1 + \gd_7} + \gd \red x} \pa{1 + \gd_8} \\
...
\begin{center}
\begin{tikzpicture}
  \draw [pattern=north east lines] (0, 0) rectangle node[above=5mm] {$M$ bits} (4, 0.5);
  \draw (4, 0) rectangle node {$b_1$} node[above=5mm] {$1$ bit} (4.5, 0.5);
  \draw [pattern=north east lines] (4.5, 0) rectangle node[fill=white] {$B$} node[above=5mm] {$M - 1$ bits} (8, 0.5);
  \draw (8, 0) rectangle node {$b_2$} node[above=5mm] {$1$ bit} (8.5, 0.5);
\end{tikzpicture}
\end{center}
...
The computation equivalent to the one described in \nameref{fmasin} is as follows:
\[
\begin{dcases}
h' &= \round{c_k - s_k h} \\
t &= h' - c_k \\
