...
In this scheme we use a two-term approximation of the modulus as $C_1 + C_2$.  Using our notation, the algorithm proceeds as follows:
\[
\begin{dcases}
...
\]
where we have used the fact that $C_1$ is very close to $\frac{\Pi}{2}$ and thus that $\ULP\of{C_1} = 2^{-M + 1}$.  The absolute error on the reduction is therefore less than:
\[
\abs{n} \; 2^{-M + 3} \ULP\of{C_1} = \abs{n} 2^{-2 M + 4}
\]

As before, the reduction guarantees $\gk_3$ extra bits of accuracy if this error is less than $2^{-\gk_3 - 1} \ULP\of{\red x}$:
...
\label{eqnboldo2}
\end{equation}
\todo{Threshold above which this reduction cannot be used.}

\subsubsection*{Boldo-Daumas-Li Argument Reduction Using a Three-Term Approximation}

If the condition above on $\abs{\red x}$ is not met, we extend the algorithm as suggested by \cite[1139]{BoldoDaumasLi2009} and \cite[205]{Muller2016}:
\[
\begin{dcases}
w &= \round{n \; C_3} \\
...
Let $\gz$ be the relative error introduced by $\dwminusfp$.  Theorem 2.2 of \cite{MullerRideau2022} indicates that $\abs{\gz} \leq 2^{1 - 2 M}$.  The result of the above computation is:
\[
\dwminusfp\of{\pa{\red x, \gd \red x}, w} = \pa{1 + \gz}\pa{x - n \pa{C_1 + C_2} - \pa{1 + \gd_1} \; n \; C_3}
\]
and the absolute error on the reduction is the difference between this quantity and $x - n \frac{\Pi}{2}$:
\begin{align*}
\mathscr{E}_{abs} &= \pa{1 + \gz}\pa{x - n \pa{C_1 + C_2} - \pa{1 + \gd_1} \; n \; C_3} - \pa{x - n \frac{\Pi}{2}} \\
&= n \pa{\frac{\Pi}{2} - \pa{C_1 + C_2 + C_3}} - n \; \gd_1 \; C_3 + \gz \pa{x - n \pa{C_1 + C_2} - \pa{1 + \gd_1} \; n \; C_3}
\end{align*}
The following inequalities hold for the various terms of the right-hand side:
\begin{align*}
\abs{\frac{\Pi}{2} - \pa{C_1 + C_2 + C_3}} &= \gd_1 \pa{\frac{\Pi}{2} - \pa{C_1 + C_2}} \leq \gd_1 \; 2^{-2 M + 4} \leq 2^{-3 M + 4} \\
\gd_1 \; C_3 &= \gd_1 \pa{1 + \gd_1} \pa{\frac{\Pi}{2} - \pa{C_1 + C_2}} \leq \gd_1 \pa{1 + \gd_1} 2^{-2 M + 4} \leq 2^{-3 M + 5} \\
\gz \pa{1 + \gd_1} C_3 &\leq \gz \pa{1 + \gd_1}^2 \; 2^{-2 M + 4} \leq 2^{-4 M + 6}\\
\gz \abs{x - n \pa{C_1 + C_2}} &= \gz \abs{\red x + \gd \red x} \leq \abs{n} 2^{\gk_3 - 3 M + 5}
\end{align*}
Where the last inequality is derived from the fact that we only use a three-term approximation when \ref{eqnboldo2} does not hold.

Putting all these inequalities together we obtain the following bound for the absolute error:
\[
\abs{\mathscr{E}_{abs}} \leq \abs{n} \pa{2^{-3 M + 4} + 2^{-3 M + 5} + 2^{-4 M + 6} + 2^{\gk_3 - 3 M + 5}}
\]
\todo{Complete the error analysis.}
