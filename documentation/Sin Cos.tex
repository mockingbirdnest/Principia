...
Finally, for error analysis we use the accuracy model of \cite{Higham2002}, equation (2.4): everywhere they appear, the quantities $\gd_i$ represent a roundoff factor such that $\gd_i < u = \ge_M / 2 = 2^{-M}$ (see pages 37 and 38).  We also use $\gq_n$ and $\gg_n$ with the same meaning as in \cite{Higham2002}, lemma 3.1.
...
\[
\abs{\hat x} \geq 2^{\gk_1 + \gk'_1 + \gk_3} \ULP\of{\frac{\Pi}{2}} = 2^{\gk_1 + \gk'_1 + \gk_3 - M + 1}
\]
...
If this condition is not met, we fall back on reducing the argument using the three-term approximation.
...
\end{align*}
A sufficient condition for the reduction to guarantee $\gk_3$ extra bits of accuracy is for this error to be less than $2^{-\gk_3} \abs{\ULP\of{\hat x}}$ which itself is less than $2^{-\gk_3 - M + 1} \abs{\hat{x}}$.  Therefore we want:
\begin{align*}
\abs{\hat x} &\geq 2^{\gk_3 - M}\pa{2^{\gk_2 + \gk'_2 + \gk''_2} \ULP\of{\frac{\Pi}{2}} + \frac{\Pi}{2}} \\
&= 2^{\gk_3 - M}\pa{2^{\gk_2 + \gk'_2 + \gk''_2 - M + 1}  + \frac{\Pi}{2}} \\
&> 2^{\gk_3 - M}\pa{2^{\gk_2 + \gk'_2 + \gk''_2 - M + 1}  + 1}
\end{align*}

If we choose $\gk_3 = 18$ as above, and $\gk_2 = 18$ we find that $\gk'_2 = 13$ and $\gk''_2 = 14$.  Therefore, the desired accuracy is obtained as long as $\abs{\hat x} \geq 129 \times 2^{-42} ≃ 2.9 \times 10^{-11}$.
