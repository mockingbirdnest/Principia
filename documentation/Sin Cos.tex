...
If $\abs{n}$ is too large (that is, if $\abs{x}$ is too large) the right-hand side of (\ref{eqnboldo2}) and (\ref{eqnboldo3}) becomes so large that no reduced angle $\red x$ is acceptable: this happens if the right-hand side reaches $\frac{\Pi}{4}$.  When deciding for which angles it is worthwhile to attempt the Boldo-Daumas-Li reduction, it is important to note that all the work done for the two-term reduction is reused by the three-term reduction, since the latter just performs more operations on the result obtained by the former.  This differs from the Cody-Waite reduction, where the result of the two-term reduction is not easily usable by the three-term reduction.  Therefore, there is not much harm in trying the two-term Boldo-Daumas-Li if there is a good probability that, if it fails, the three-term reduction will succeed.
...
\]
whose numerical value is approximately $4.6 \times 10^{-11}$.  There is therefore a high probability that, even if the two-term reduction fails, the three-term reduction will succeed and therefore the work done by the two-term reduction will not be wasted.
