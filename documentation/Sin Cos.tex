...
e_{FMA} &\DefineAs 1 + \frac{1 + 2^{M + 1} \; \overbar\ge_1}{1 - \overbar\ge_1 - 2^{-k + 1}}
\end{align*}
...
\[
e_{NoFMA} \DefineAs \pa{1 - 2^{-M}}^{-1}\pa{1 + \frac{1 + 2^{M + 1} \; \overbar\ge_1}{1 - \overbar\ge_1 - 2^{-k + 1}}}
\]
We see immediately that $e_{FMA} < e_{NoFMA}$, and it is therefore correct to perform the rounding test using $e_{NoFMA}$, even if an FMA is used to compute $\round{y + \gd y \; e}$.

Obviously, if the above expression does not produce a machine number, it must be rounded towards $+\infty$.  In each of the error analyses below we document the concrete factor $\roundTowardPositive{e_{NoFMA}}$ to use for the rounding test.
