...
\label{eqnpitwoterms}
\end{align}
With the chosen value $\gk_1 = 8$, the two sides of this inequality turn out to be $2^{-103.217}$ and $2^{-101}$.  The difference comes from the fact that $\abs{\frac{\Pi}{2} - C_1 - \gd C_1}$ has three zeroes after the last bit of its mantissa, but the theoretical computation above assumes the worse, i.e., a run of ones.  In the rest of our discussion of approximations of $\Pi$ we will give the (tighter) error bounds obtained by computing the left-hand side with the actual value of $\Pi$.
...
\label{eqnabsdc1}
\end{align}
With the chosen value of $\gk_1$, and using the actual value of $\Pi$, this quantity is found to be less than $2^{-47.398}$.
...
\end{align*}
With the chosen value of $\gk_2$, and using the actual value of $\Pi$, this quantity is found to be less than $2^{-130.477}$.
...
\label{eqnabsdc2}
\end{equation}
With the chosen value of $\gk_2$, and using the actual value of $\Pi$, this quantity is found to be less than $2^{-76.325}$.
...
\end{align*}
where we have used the upper bound for $\gd C_1$ given by equation (\ref{eqnabsdc1}).  With the chosen value of $\gk_1$, and using the actual value of $\Pi$, this quantity is found to be less than $2^{-92.719}$.
...
\end{align*}
where the second inequality uses $\gg_2 \pa{2^{\gk_2 + 1} + 1} < 3 u \; 2^{\gk_2 + 1}$.  In practice this quantity is smaller than $2^{-104.336}$.
