...
This section documents the core of the algorithm.  It takes as input the result of argument reduction, $\pa{\red x, \gd \red x}$\marginnote{TODO(phl): Decide what to do with the hats.} and produces a pair $\pa{y, \gd y}$ which is passed to the rounding test described in \cite[397]{MullerBrisebarreDeDinechinJeannerodLefevreMelquiondRevolStehleTorres2010} to decide whether we need to fall back to the CORE-MATH implementation.  Most of the time (with a probability close to $1 - 2^{-18}$) $y$ is the correctly-rounded result.

In this analysis we assume that $\red x$ is positive ($\gd \red x$ may be positive or negative).  We also assume the availability of an FMA operation, and the rounding direction $roundTiesToEven$.  The evaluation of the polynomials $p_{s0}$, $p_s$, and $p_c$ has two sources of errors: the rounding of the coefficients to machine numbers, and the error due to the floating-point operations.  The error interval is computed using the functions \texttt{IEEEEvaluateWithRelativeError} and \texttt{IEEEEvaluateWithAbsoluteError} in file \linebreak\href{https://github.com/mockingbirdnest/Principia/blob/master/mathematica/ieee754_floating_point_evaluation.wl}{\texttt{mathematica/ieee754\_floating\_point\_evaluation.wl}}.  It is generally asymmetrical because rounding the coefficients introduces a bias.  We use $\roundAll{expr}$ to denote evaluation where appropriate rounding happens on each literal or operation of the expression $expr$.\marginnote{TODO(phl): Try to tune the high-degree coefficient after rounding the low-degree one.}
...
First note that, in the error analysis (and in the computation itself) the terms ${\red x}^n \; \gd \red x$ for $n > 0$ are irrelevant.  The reason is that $\abs{\red x \; \gd \red x} < \gD^2 / 2^M = 2^{-M - 20}$.  But we chose $\gk_3 = 18$ so we are already willing to tolerate errors significantly larger than $\red x \; \gd \red x$.  This is why we do not involve $\gd \red x$ in steps $t_1$ and $t_2$ above: the most significant term of $t_1 t_2$ would be ${\red x}^3 \; \gd \red x$.
...
&= \pa{\pa{1 + \gz_1} \pa{1 + \gz_2} \pa{1 + \gz_3} \pa{\sin \red x - \red x} + \gd \red x} &\pa{1 + \gd_1}&
\end{alignedat}
\right.
\]
