...
In this analysis we assume that $\hat x$ is positive ($\gd \hat x$ may be positive or negative).  We also assume the availability of an FMA operation, and the rounding direction $roundTiesToEven$.  The evaluation of the polynomials $p_{s0}$, $p_s$, and $p_c$ has two sources of errors: the rounding of the coefficients to machine numbers, and the error due to the floating-point operations.  The error interval is computed using the functions \texttt{IEEEEvaluateWithRelativeError} and \texttt{IEEEEvaluateWithAbsoluteError} in file \linebreak\texttt{mathematica/ieee754\_floating\_point\_evaluation.wl}.  It is generally asymmetrical because rounding the coefficients introduces a bias.  We use $\roundAll{expr}$ to denote evaluation where appropriate rounding happens on each literal or operation of the expression $expr$.\marginnote{TODO(phl): Try to tune the high-degree coefficient after rounding the low-degree one.}
...
\[
\begin{dcases}
t_1\of{h, \gd x} &\DefineAs \round{h + 2 \gd x} \\
t_2\of{h, \gd x} &\DefineAs \round{h \; t_1\of{h, \gd x}} \\
t_3\of{h, \gd x} &\DefineAs \round{t_2\of{h, \gd x} h} \\
t_4\of{h, \gd x} &\DefineAs \round{p_s\of{t_2\of{h, \gd x}}} \\
t_5\of{h, \gd x} &\DefineAs \round{t_3\of{h, \gd h} \; t_4\of{h, \gd x}} \\
t_6\of{h, \gd x} &\DefineAs \round{p_c\of{t_2\of{h, \gd x}}} \\
t_7\of{h, \gd x} &\DefineAs \round{t_2\of{h} \; t_6\of{h, \gd x}} \\
t_8\of{h, \gd x} &\DefineAs \round{s_k \; t_7\of{h, \gd x}} \\
t_9\of{h, \gd x} &\DefineAs \round{c_k \; t_5\of{h, \gd x} + t_8\of{h, \gd x}} \\
t_{10}\of{h, \gd x} &\DefineAs \round{t_9\of{h, \gd x} + \gd z} \\
y &= z \\
