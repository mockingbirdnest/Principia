...
Euler's equation $\TimeDerivative{\vm} = \vm\Exterior\pa{\VectorSymbol{I}^{-1} \vm}$ can be written in coordinates:
\[
\TimeDerivative{\vm} =
\begin{pmatrix}
m_1 \\ m_2 \\ m_3
...
\end{align*}
It is easy to verify that the argument of the $\HyperbolicTangent^{-1}$ is dimensionless and that $\gl_2$ has the same dimension as the quotient 
