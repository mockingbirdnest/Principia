...
\sisetup{mode = text, math-rm = \mathscr, power-font = number}
...
\begin{align*}
I_{jh} &\DefineAs I_j - I_h &\gD_j &\DefineAs G^2 - 2 T I_j\si{\radian\squared} &B_{jh} &\DefineAs \sqrt{±\frac{I_j \gD_h}{I_{jh}}} \\
...
\end{align*}
Now note that in case (iii) we have $2 T I_2 \si{\radian\squared} = G^2$ thus $1/I_2 = 2 T \si{\radian\squared}/G^2$.  $\gl$ can be rewritten as:
\[
\gl = \gs' \gs'' G \sqrt{-\frac{\gD_1}{I_1 I_3 \gD_3}} \pa{\frac{2 T I_3 \si{\radian\squared}}{G^2} - 1} = \gs' \gs'' \frac{1}{G} \sqrt{-\frac{\gD_1 \gD_3}{I_1 I_3}}
\]
where we have used the fact that $2 T I_3 \si{\radian\squared} - G^2 = -\gD_3 = 2 T \si{\radian\squared} \pa{I_3 - I_2} ≥ 0$.
...
2 T \si{\radian\squared} &= \frac{m_1^2}{I_1} + \frac{m_2^2}{I_2} + \frac{m_3^2}{I_3}
\end{dcases}
\]
...
They then define active rotations in the physical (vector) space.  For instance they explain that $\mathscr P_t$ takes $\VectorSymbol M$ to ${\ve}_3$ and transforms the basis $\mathscr B_t$ into the basis $\mathscr B^b$.  By contrast, our libraries operate on coordinate representations, not
abstract vectors, and implement passive rotations where the physical space is represented by multiple copies of $\Reals^3$ with different coordinate
systems.  Therefore, we view $\mathscr P_t$ as transforming $\vm$ in the coordinate system $\mathscr B^b$ of the body into $\vm$ in the
coordinate system $\mathscr B_t$.  Confusingly, \cite{Celledoni2007} appear to use passive rotations when they write matrices, so their $P_t$ has
semantics similar to that of our $\mathscr P_t$.
...
\begin{align*}
\TimeDerivative{\gy}\of{t} &= \frac{2 T \si{\radian\squared}}{G} + \frac{\gD_2}{G I_2}\pa{\frac{1}{1 + \frac{I_{12}I_{23}G^2}{I_2^2 \gD_1 \gD_3}m_2^2}}\\
&= \frac{2 T \si{\radian\squared}}{G} + \frac{\gD_2}{G I_2}\pa{\frac{1}{1 - \frac{G^2}{B_{21}^2 B_{23}^2}m_2^2}}
\end{align*}

\subsection*{Case (i)}
In case (i) we have $m_2 = -B_{21} \JacobiSN\of{\gl t - \gn, k}$ and the above expression becomes:
\[
\TimeDerivative{\gy}\of{t} = \frac{2 T \si{\radian\squared}}{G} + \frac{\gD_2}{G I_2}\pa{\frac{1}{1 - \frac{G^2}{B_{23}^2}\JacobiSN^2\of{\gl t - \gn, k}}}
\]
This expression can be integrated using formula 110.04 of \cite{ByrdFriedman1954} with $\ga = G/B_{23}$ to yield:
\[
\gy\of{t} = \frac{2 T \si{\radian\squared}}{G}t + \frac{\gD_2}{\gl G I_2}\EllipticPi\of{\JacobiAmplitude\of{\gl t - \gn, k}, \frac{G^2}{B_{23}^2}, k}
\]
...
\[
\TimeDerivative{\gy}\of{t} = \frac{2 T \si{\radian\squared}}{G} + \frac{\gD_2}{G I_2}\pa{\frac{1}{1 - \frac{G^2}{B_{21}^2}\JacobiSN^2\of{\gl t - \gn, k^{-1}}}}
\]
and:
\[
\gy\of{t} = \frac{2 T \si{\radian\squared}}{G}t + \frac{\gD_2}{\gl G I_2}\EllipticPi\of{\JacobiAmplitude\of{\gl t - \gn, k^{-1}}, \frac{G^2}{B_{21}^2}, k^{-1}}
\]

\subsection*{Case (iii)}
In case (iii) we have $m_2 = G \HyperbolicTangent\of{\gl t - \gn}$ and:
\[
\TimeDerivative{\gy}\of{t} = \frac{2 T \si{\radian\squared}}{G} + \frac{\gD_2}{G I_2}\pa{\frac{1}{1 - \frac{G^4}{B_{21}^2 B_{23}^2} \HyperbolicTangent^2\of{\gl t - \gn}}}
\]
...
\[
\TimeDerivative{\gy}\of{t} = \frac{2 T \si{\radian\squared} + G m_3 / I_3}{G + m_3} + 4 G \frac{p_3 \TimeDerivative{p_0} - p_0 \TimeDerivative{p_3}}{G + m_3}
\]
...
\[
\TimeDerivative{\gy}\of{t} = \frac{2 T \si{\radian\squared} + G m_1 / I_1}{G + m_1} + 4 G \frac{p_1 \TimeDerivative{p_0} - p_0 \TimeDerivative{p_1}}{G + m_1}
\]
...
and the angle to:
\[
\TimeDerivative{\gy}\of{t} = \frac{2 T \si{\radian\squared} + G m_1 / I_1}{G + m_1} = \frac{G^2 - \gD_1 + G m_1}{I_1\pa{G + m_1}} = \frac{G}{I_1} - \frac{\gD_1/I_1}{G + m_1} =
