...
describe in this section the modifications we make to their formul{\ae} in our implementation.  We also restore dimensionful formul{\ae} as needed.

First, it is simpler and more efficient to avoid absolute values, so we define:
\[
...
Next we notice that the computation of $\gD_j$ may entail cancellations, so we go back to the definition of $\norm{\vm}$ and of the kinetic energy:
\[
\begin{dcases}
...
\end{dcases}
\]
and we define $\gD_j$ without absolute values:
\[
...
\end{dcases}
\]
It is easy to see that $\gD_1$ and $\gD_3$ are the sums of terms of the same sign, so they can be computed without cancellations and $\gD_1 \geq 0$ and 
$\gD_3 \leq 0$.  $\gD_2$ can have either sign, which correspond exactly to cases (i) ($\gD_2 < 0$), (ii) ($\gD_2 > 0$) and (iii) ($\gD_2 = 0$).

Finally, for the computation of the elliptic functions and integrals \cite{Celledoni2008} gives the value of $k$ but we need the value of $m_c = 1 - m$ 
(see \cite{NistHMF2010}, section 19.1.2 for an overview of the notation).  In case (i) we have:
\[
