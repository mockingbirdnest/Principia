...
Unfortunately, it is not suitable for a practical implementation because it has an essential singularity when $\TimeDerivative{\vm} = \v0$:
while the condition $\TimeDerivative{\vm} = \v0$ is physically a constant of motion, $\TimeDerivative{\vm}$ itself is not.  This means that
any inaccuracies in numerical computations of $\TimeDerivative{\vm}$ (cancellations, underflow) may cause it to switch from $\v0$ to non-$\v0$ and back.

When $\TimeDerivative{\vm} = \v0$ at $t = 0$, the motion may be computed at all times assuming a constant $\vm$, so the singularity may be
eliminated.  But when $\TimeDerivative{\vm} \neq \v0$ at $t = 0$ and it later becomes $\v0$, there is no way to find a basis at $t$ that 
...
We start with computing two integrals that are useful in the computation of $\gy$.  As far as we can tell the following integral is missing from
\cite{ByrdFriedman1954}:
\[
\int{}\frac{1}{1 - a \JacobiDN\of{u, k}}\diffd{u} = \\
\frac{1}{1 - a^2}\pa{\EllipticPi\of{\JacobiAmplitude\of{u, k}, \frac{a^2 m}{a^2 - 1}, k} - 
\sqrt\frac{a \pa{1 - a^2}}{a^2\pa{m - 1} + 1} \InverseTrigonometricTangent\of{\sqrt\frac{a^2 \pa{m - 1} + 1}{1 - a^2} \JacobiSC\of{u, k}}}
\]
