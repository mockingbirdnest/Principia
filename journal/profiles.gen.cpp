...
void AddVesselToNextPhysicsBubble::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  auto const plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const vessel_guid = in.vessel_guid().c_str();
  auto const parts = [](::google::protobuf::RepeatedPtrField<serialization::KSPPart> const& messages) -> std::vector<KSPPart> {
...
void AdvanceTime::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  auto const plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const t = in.t();
  auto const planetarium_rotation = in.planetarium_rotation();
...
void AtEnd::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const line_and_iterator = DeserializePointer<LineAndIterator const*>(*pointer_map, in.line_and_iterator());
...
void BubbleDisplacementCorrection::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto const sun_position = DeserializeXYZ(in.sun_position());
...
void BubbleVelocityCorrection::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto const reference_body_index = in.reference_body_index();
...
void CelestialFromParent::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto const celestial_index = in.celestial_index();
...
void DeleteLineAndIterator::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const line_and_iterator = DeserializePointer<LineAndIterator*>(*pointer_map, in.line_and_iterator());
...
void DeletePluginSerialization::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const serialization = DeserializePointer<char const*>(*pointer_map, in.serialization());
...
void DeserializePlugin::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const serialization = &in.serialization();
  auto const deserializer = DeserializePointer<PushDeserializer*>(*pointer_map, in.deserializer());
  auto const plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
...
void DirectlyInsertCelestial::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  auto const plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const celestial_index = in.celestial_index();
  auto const parent_index = in.has_parent_index() ? std::make_unique<int const>(in.parent_index()) : nullptr;
  auto const gravitational_parameter = in.gravitational_parameter().c_str();
  auto const axis_right_ascension = in.has_axis_right_ascension() ? in.axis_right_ascension().c_str() : nullptr;
  auto const axis_declination = in.has_axis_declination() ? in.axis_declination().c_str() : nullptr;
  auto const j2 = in.has_j2() ? in.j2().c_str() : nullptr;
  auto const reference_radius = in.has_reference_radius() ? in.reference_radius().c_str() : nullptr;
  auto const x = in.x().c_str();
  auto const y = in.y().c_str();
  auto const z = in.z().c_str();
  auto const vx = in.vx().c_str();
  auto const vy = in.vy().c_str();
  auto const vz = in.vz().c_str();
...
void InsertCelestial::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  auto const plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const celestial_index = in.celestial_index();
  auto const gravitational_parameter = in.gravitational_parameter();
  auto const parent_index = in.parent_index();
  auto const from_parent = DeserializeQP(in.from_parent());
...
void NavballOrientation::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto const sun_world_position = DeserializeXYZ(in.sun_world_position());
  auto const ship_world_position = DeserializeXYZ(in.ship_world_position());
...
void NewBarycentricRotatingNavigationFrame::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto const primary_index = in.primary_index();
  auto const secondary_index = in.secondary_index();
...
void NewPlugin::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const initial_time = in.initial_time();
  auto const planetarium_rotation_in_degrees = in.planetarium_rotation_in_degrees();
...
void RenderedFlightPlan::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  auto const plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const vessel_guid = in.vessel_guid().c_str();
  auto const plan_phase = in.plan_phase();
  auto const sun_world_position = DeserializeXYZ(in.sun_world_position());
...
void SerializePlugin::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto const serializer = DeserializePointer<PullSerializer*>(*pointer_map, in.serializer());
...
void SetBufferedLogging::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const max_severity = in.max_severity();
...
void SetPlottingFrame::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const navigation_frame = DeserializePointer<NavigationFrame*>(*pointer_map, in.navigation_frame());
...
void SetStderrLogging::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto const min_severity = in.min_severity();
