...
void BM_PlayForReal(benchmark::State& state) {
  while (state.KeepRunning()) {
    Player player(
        R"(P:\Public Mockingbird\Principia\Journals\JOURNAL.20170520-201036)");
    int count = 0;
    while (player.Play()) {
      ++count;
...
TEST_F(PlayerTest, DISABLED_Debug) {
  ...
  std::string path =
      R"(C:\Program Files\Kerbal Space Program\1.2.2\glog\Principia\JOURNAL.20170604-182825)";  // NOLINT
  Player player(path);
  int count = 0;
  while (player.Play()) {
    ++count;
