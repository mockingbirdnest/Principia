...
void BM_PlayForReal(benchmark::State& state) {
  while (state.KeepRunning()) {
    Player player(
        R"(P:\Public Mockingbird\Principia\Journals\JOURNAL.20170813-172910)");
    int count = 0;
    while (player.Play()) {
      ++count;
...
TEST_F(PlayerTest, DISABLED_Debug) {
  ...
  std::string path =
      R"(\\venezia.mockingbirdnest.com\Namespaces\Public\Public Mockingbird\Principia\Crashes\1553\JOURNAL.20170908-233206)";  // NOLINT
  Player player(path);
  int count = 0;
  while (player.Play()) {
    ++count;
