...
OrbitAnalysis DeserializeOrbitAnalysis(serialization::OrbitAnalysis const& orbit_analysis, Player::PointerMap& pointer_map, int& primary_index_storage, OrbitalElements& elements_storage, OrbitRecurrence& recurrence_storage, OrbitGroundTrack& ground_track_storage) {
...
void FlightPlanGetCoastAnalysis::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  PRINCIPIA_CHECK_EQ(DeserializeOrbitAnalysis(message.return_().result(), pointer_map, primary_index_storage, elements_storage, recurrence_storage, ground_track_storage), *result);
  Insert(message.return_().address(), result, pointer_map);
}
...
void PlanetariumCreate::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  auto const result = interface::principia__PlanetariumCreate(plugin, sun_world_position, xyz_opengl_camera_x_in_world, xyz_opengl_camera_y_in_world, xyz_opengl_camera_z_in_world, xyz_camera_position_in_world, focal, field_of_view, inverse_scale_factor, scaled_space_origin);
  Insert(message.return_().result(), result, pointer_map);
}
...
void PlanetariumPlotFlightPlanSegment::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  [[maybe_unused]] auto const& out = message.out();
  int vertex_count;
  interface::principia__PlanetariumPlotFlightPlanSegment(planetarium, plugin, vessel_guid, index, vertices, vertices_size, &vertex_count);
}
...
void PlanetariumPlotPrediction::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  [[maybe_unused]] auto const& out = message.out();
  int vertex_count;
  interface::principia__PlanetariumPlotPrediction(planetarium, plugin, vessel_guid, vertices, vertices_size, &vertex_count);
}
...
void PlanetariumPlotPsychohistory::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  [[maybe_unused]] auto const& out = message.out();
  int vertex_count;
  interface::principia__PlanetariumPlotPsychohistory(planetarium, plugin, vessel_guid, max_history_length, vertices, vertices_size, &vertex_count);
}
