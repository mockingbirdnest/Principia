...
void FlightPlanGetFinalTime::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_plugin(SerializePointer(in.plugin));
  m->set_vessel_guid(in.vessel_guid);
}

void FlightPlanGetFinalTime::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void FlightPlanGetFinalTime::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
  auto const result = interface::principia__FlightPlanGetFinalTime(plugin, vessel_guid);
  CHECK(message.return_().result() == result);
}
...
void FlightPlanRenderedSegmentEndpoints::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_plugin(SerializePointer(in.plugin));
  m->set_vessel_guid(in.vessel_guid);
  *m->mutable_sun_world_position() = SerializeXYZ(in.sun_world_position);
...
void FlightPlanRenderedSegmentEndpoints::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeXYZSegment(result);
}

void FlightPlanRenderedSegmentEndpoints::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
  auto sun_world_position = DeserializeXYZ(in.sun_world_position());
  auto index = in.index();
  auto const result = interface::principia__FlightPlanRenderedSegmentEndpoints(plugin, vessel_guid, sun_world_position, index);
  CHECK(DeserializeXYZSegment(message.return_().result()) == result);
}
...
void GetVersion::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& out = message.out();
  auto build_date = DeserializePointer<char const*>(*pointer_map, out.build_date());
  auto version = DeserializePointer<char const*>(*pointer_map, out.version());
...
void InsertCelestialAbsoluteCartesian::Fill(In const& in, not_null<Message*> const message) {
  ...
  m->set_mean_radius(in.mean_radius);
...
void InsertCelestialAbsoluteCartesian::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  auto mean_radius = in.mean_radius().c_str();
  ...
  auto vy = in.vy().c_str();
  auto vz = in.vz().c_str();
  interface::principia__InsertCelestialAbsoluteCartesian(plugin, celestial_index, parent_index.get(), gravitational_parameter, mean_radius, axis_right_ascension, axis_declination, j2, reference_radius, x, y, z, vx, vy, vz);
}
...
void InsertCelestialJacobiKeplerian::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  interface::principia__InsertCelestialJacobiKeplerian(plugin, celestial_index, parent_index, gravitational_parameter, mean_radius, axis_right_ascension, axis_declination, j2, reference_radius, eccentricity, mean_motion, inclination, longitude_of_ascending_node, argument_of_periapsis, mean_anomaly);
}
...
void InsertSun::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  ...
  auto mean_radius = in.mean_radius();
  interface::principia__InsertSun(plugin, celestial_index, gravitational_parameter, mean_radius);
}
