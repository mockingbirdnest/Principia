...
void CelestialInitialRotationInDegrees::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto celestial_index = in.celestial_index();
...
void CelestialRotation::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto index = in.index();
...
void CelestialRotationPeriod::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto celestial_index = in.celestial_index();
...
void CelestialSphereRotation::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
...
void DeleteString::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
...
void FlightPlanGetAdaptiveStepParameters::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
...
void FlightPlanGetActualFinalTime::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
...
void FlightPlanGetDesiredFinalTime::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
...
void FlightPlanGetInitialTime::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
...
void FlightPlanRenderedApsides::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
  auto celestial_index = in.celestial_index();
...
void FlightPlanSetAdaptiveStepParameters::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
...
void FlightPlanSetDesiredFinalTime::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
  auto final_time = in.final_time();
...
void HasEncounteredApocalypse::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const& out = message.out();
  char const* details;
...
void IsKspStockSystem::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_plugin(SerializePointer(in.plugin));
}

void IsKspStockSystem::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void IsKspStockSystem::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto const result = interface::principia__IsKspStockSystem(plugin);
  CHECK(message.return_().result() == result);
}
...
void IteratorAtEnd::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
...
void IteratorDelete::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
...
void IteratorGetQP::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_iterator(SerializePointer(in.iterator));
}

void IteratorGetQP::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeQP(result);
}

void IteratorGetQP::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(*pointer_map, in.iterator());
  auto const result = interface::principia__IteratorGetQP(iterator);
  CHECK(DeserializeQP(message.return_().result()) == result);
}

void IteratorGetTime::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_iterator(SerializePointer(in.iterator));
}

void IteratorGetTime::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void IteratorGetTime::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(*pointer_map, in.iterator());
...
void IteratorGetXYZ::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeXYZ(result);
}

void IteratorGetXYZ::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(*pointer_map, in.iterator());
...
void IteratorIncrement::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator*>(*pointer_map, in.iterator());
...
void IteratorSize::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(*pointer_map, in.iterator());
...
void PhysicsBubbleDisplacementCorrection::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_plugin(SerializePointer(in.plugin));
  *m->mutable_sun_position() = SerializeXYZ(in.sun_position);
}

void PhysicsBubbleDisplacementCorrection::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeXYZ(result);
}

void PhysicsBubbleDisplacementCorrection::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto sun_position = DeserializeXYZ(in.sun_position());
  auto const result = interface::principia__PhysicsBubbleDisplacementCorrection(plugin, sun_position);
  CHECK(DeserializeXYZ(message.return_().result()) == result);
}

void PhysicsBubbleVelocityCorrection::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_plugin(SerializePointer(in.plugin));
  m->set_reference_body_index(in.reference_body_index);
}

void PhysicsBubbleVelocityCorrection::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeXYZ(result);
}

void PhysicsBubbleVelocityCorrection::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto reference_body_index = in.reference_body_index();
  auto const result = interface::principia__PhysicsBubbleVelocityCorrection(plugin, reference_body_index);
  CHECK(DeserializeXYZ(message.return_().result()) == result);
}
...
void RenderedPredictionApsides::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
  auto celestial_index = in.celestial_index();
...
void SetMainBody::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin*>(*pointer_map, in.plugin());
  auto index = in.index();
...
void VesselGetPredictionAdaptiveStepParameters::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
...
void VesselSetPredictionAdaptiveStepParameters::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
  auto adaptive_step_parameters = DeserializeAdaptiveStepParameters(in.adaptive_step_parameters());
...
void VesselVelocity::Run(Message const& message, not_null<Player::PointerMap*> const pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(*pointer_map, in.plugin());
  auto vessel_guid = in.vessel_guid().c_str();
