...
BodyParameters DeserializeBodyParameters(serialization::BodyParameters const& body_parameters, Player::PointerMap& pointer_map, std::pair<std::vector<BodyGeopotentialElement>, std::vector<BodyGeopotentialElement const*>>& geopotential_storage) {
  return {body_parameters.name().c_str(),
          ...
          body_parameters.has_atmosphere_depth() ? body_parameters.atmosphere_depth().c_str() : nullptr,
          body_parameters.has_ocean(),
...
ConfigurationDownsamplingParameters DeserializeConfigurationDownsamplingParameters(serialization::ConfigurationDownsamplingParameters const& configuration_downsampling_parameters, Player::PointerMap& pointer_map) {
  return {configuration_downsampling_parameters.tolerance().c_str()};
}
...
PlottableElements DeserializePlottableElements(serialization::PlottableElements const& plottable_elements, Player::PointerMap& pointer_map) {
  return {plottable_elements.time(),
          plottable_elements.semimajor_axis(),
          plottable_elements.eccentricity(),
          plottable_elements.inclination(),
          plottable_elements.longitude_of_ascending_node(),
          plottable_elements.argument_of_periapsis(),
          plottable_elements.periapsis_distance(),
          plottable_elements.apoapsis_distance(),
          plottable_elements.lidov_c1(),
          plottable_elements.lidov_c2(),
          plottable_elements.eccentricity_cos_argument_of_periapsis(),
          plottable_elements.eccentricity_sin_argument_of_periapsis()};
}
...
OrbitalElements DeserializeOrbitalElements(serialization::OrbitalElements const& orbital_elements, Player::PointerMap& pointer_map, double& first_collision_time_storage, double& first_collision_risk_time_storage, double& first_reentry_time_storage) {
  return {orbital_elements.sidereal_period(),
          ...
          DeserializePointer<Iterator*>(orbital_elements.plottable_elements(), pointer_map)};
}
...
serialization::BodyParameters SerializeBodyParameters(BodyParameters const& body_parameters) {
  ...
  if (body_parameters.atmosphere_depth != nullptr) {
    m.set_atmosphere_depth(body_parameters.atmosphere_depth);
  }
  m.set_has_ocean(body_parameters.has_ocean);
...
serialization::PlottableElements SerializePlottableElements(PlottableElements const& plottable_elements) {
  serialization::PlottableElements m;
  m.set_time(plottable_elements.time);
  m.set_semimajor_axis(plottable_elements.semimajor_axis);
  m.set_eccentricity(plottable_elements.eccentricity);
  m.set_inclination(plottable_elements.inclination);
  m.set_longitude_of_ascending_node(plottable_elements.longitude_of_ascending_node);
  m.set_argument_of_periapsis(plottable_elements.argument_of_periapsis);
  m.set_periapsis_distance(plottable_elements.periapsis_distance);
  m.set_apoapsis_distance(plottable_elements.apoapsis_distance);
  m.set_lidov_c1(plottable_elements.lidov_c1);
  m.set_lidov_c2(plottable_elements.lidov_c2);
  m.set_eccentricity_cos_argument_of_periapsis(plottable_elements.eccentricity_cos_argument_of_periapsis);
  m.set_eccentricity_sin_argument_of_periapsis(plottable_elements.eccentricity_sin_argument_of_periapsis);
  return m;
}
...
serialization::OrbitalElements SerializeOrbitalElements(OrbitalElements const& orbital_elements) {
  ...
  *m.mutable_radial_distance() = SerializeInterval(orbital_elements.radial_distance);
  ...
  m.set_plottable_elements(SerializePointer(orbital_elements.plottable_elements));
  return m;
}
...
void InsertOrbitalElements(serialization::OrbitalElements const& orbital_elements_proto, OrbitalElements const& orbital_elements_object, Player::PointerMap& pointer_map) {
  ...
  Insert(orbital_elements_proto.plottable_elements(), orbital_elements_object.plottable_elements, pointer_map);
}
...
void CollisionDeleteExecutor::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin const*>(in.plugin(), pointer_map);
  auto executor = DeserializePointer<PushPullExecutor<std::optional<DistinguishedPoints<World>::value_type>, Length, Angle, Angle>*>(in.executor(), pointer_map);
  [[maybe_unused]] auto const& out = message.out();
  TQP collision;
...
void DeserializePlugin::Fill(In const& in, not_null<Message*> const message) {
  ...
  m->set_plugin_reader(SerializePointer(*in.plugin_reader));
...
void DeserializePlugin::Fill(Out const& out, not_null<Message*> const message) {
  ...
  m->set_plugin_reader(SerializePointer(*out.plugin_reader));
}
...
void DeserializePlugin::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  auto plugin_reader = DeserializePointer<PluginReader*>(in.plugin_reader(), pointer_map);
  auto compressor = in.compressor().c_str();
  auto encoder = in.encoder().c_str();
  [[maybe_unused]] auto const& out = message.out();
  interface::principia__DeserializePlugin(serialization, &deserializer, &plugin_reader, compressor, encoder);
  ...
  Insert(out.plugin_reader(), plugin_reader, pointer_map);
}
...
void GetVersion::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  char const* version;
  char const* platform;
  interface::principia__GetVersion(&build_date, &version, &platform);
}

void GraphLidovFrozenLine::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_c2(in.c2);
}

void GraphLidovFrozenLine::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void GraphLidovFrozenLine::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto c2 = in.c2();
  auto const result = interface::principia__GraphLidovFrozenLine(c2);
  PRINCIPIA_CHECK_EQ(message.return_().result(), result);
}

void GraphLidovMaximalEccentricityLine::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_eccentricity(in.eccentricity);
  m->set_c2(in.c2);
}

void GraphLidovMaximalEccentricityLine::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void GraphLidovMaximalEccentricityLine::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto eccentricity = in.eccentricity();
  auto c2 = in.c2();
  auto const result = interface::principia__GraphLidovMaximalEccentricityLine(eccentricity, c2);
  PRINCIPIA_CHECK_EQ(message.return_().result(), result);
}

void GraphLidovMaximalEccentricityLineC2Range::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_eccentricity(in.eccentricity);
}

void GraphLidovMaximalEccentricityLineC2Range::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeInterval(result);
}

void GraphLidovMaximalEccentricityLineC2Range::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto eccentricity = in.eccentricity();
  auto const result = interface::principia__GraphLidovMaximalEccentricityLineC2Range(eccentricity);
  PRINCIPIA_CHECK_EQ(DeserializeInterval(message.return_().result(), pointer_map), result);
}

void GraphLidovMaximalInclinationLine::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_inclination_in_degrees(in.inclination_in_degrees);
  m->set_c2(in.c2);
}

void GraphLidovMaximalInclinationLine::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void GraphLidovMaximalInclinationLine::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto inclination_in_degrees = in.inclination_in_degrees();
  auto c2 = in.c2();
  auto const result = interface::principia__GraphLidovMaximalInclinationLine(inclination_in_degrees, c2);
  PRINCIPIA_CHECK_EQ(message.return_().result(), result);
}

void GraphLidovMaximalInclinationLineC2Range::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_inclination_in_degrees(in.inclination_in_degrees);
}

void GraphLidovMaximalInclinationLineC2Range::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeInterval(result);
}

void GraphLidovMaximalInclinationLineC2Range::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto inclination_in_degrees = in.inclination_in_degrees();
  auto const result = interface::principia__GraphLidovMaximalInclinationLineC2Range(inclination_in_degrees);
  PRINCIPIA_CHECK_EQ(DeserializeInterval(message.return_().result(), pointer_map), result);
}

void GraphLidovMinimalEccentricityLeftLine::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_eccentricity(in.eccentricity);
  m->set_c2(in.c2);
}

void GraphLidovMinimalEccentricityLeftLine::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void GraphLidovMinimalEccentricityLeftLine::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto eccentricity = in.eccentricity();
  auto c2 = in.c2();
  auto const result = interface::principia__GraphLidovMinimalEccentricityLeftLine(eccentricity, c2);
  PRINCIPIA_CHECK_EQ(message.return_().result(), result);
}

void GraphLidovMinimalEccentricityLeftLineC2Range::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_eccentricity(in.eccentricity);
}

void GraphLidovMinimalEccentricityLeftLineC2Range::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeInterval(result);
}

void GraphLidovMinimalEccentricityLeftLineC2Range::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto eccentricity = in.eccentricity();
  auto const result = interface::principia__GraphLidovMinimalEccentricityLeftLineC2Range(eccentricity);
  PRINCIPIA_CHECK_EQ(DeserializeInterval(message.return_().result(), pointer_map), result);
}

void GraphLidovMinimalEccentricityRightLineC2AndC1Max::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_eccentricity(in.eccentricity);
}

void GraphLidovMinimalEccentricityRightLineC2AndC1Max::Fill(Out const& out, not_null<Message*> const message) {
  auto* const m = message->mutable_out();
  m->set_c2(*out.c2);
  m->set_c1_max(*out.c1_max);
}

void GraphLidovMinimalEccentricityRightLineC2AndC1Max::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto eccentricity = in.eccentricity();
  [[maybe_unused]] auto const& out = message.out();
  double c2;
  double c1_max;
  interface::principia__GraphLidovMinimalEccentricityRightLineC2AndC1Max(eccentricity, &c2, &c1_max);
}

void GraphLidovMinimalInclinationLine::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_inclination_in_degrees(in.inclination_in_degrees);
  m->set_c2(in.c2);
}

void GraphLidovMinimalInclinationLine::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void GraphLidovMinimalInclinationLine::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto inclination_in_degrees = in.inclination_in_degrees();
  auto c2 = in.c2();
  auto const result = interface::principia__GraphLidovMinimalInclinationLine(inclination_in_degrees, c2);
  PRINCIPIA_CHECK_EQ(message.return_().result(), result);
}

void GraphLidovMinimalInclinationLineC2Range::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_inclination_in_degrees(in.inclination_in_degrees);
}

void GraphLidovMinimalInclinationLineC2Range::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeInterval(result);
}

void GraphLidovMinimalInclinationLineC2Range::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto inclination_in_degrees = in.inclination_in_degrees();
  auto const result = interface::principia__GraphLidovMinimalInclinationLineC2Range(inclination_in_degrees);
  PRINCIPIA_CHECK_EQ(DeserializeInterval(message.return_().result(), pointer_map), result);
}
...
void IteratorGetDistinguishedPointsQP::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_iterator(SerializePointer(in.iterator));
}

void IteratorGetDistinguishedPointsQP::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeQP(result);
}

void IteratorGetDistinguishedPointsQP::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(in.iterator(), pointer_map);
  auto const result = interface::principia__IteratorGetDistinguishedPointsQP(iterator);
  PRINCIPIA_CHECK_EQ(DeserializeQP(message.return_().result(), pointer_map), result);
}

void IteratorGetDistinguishedPointsTime::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_iterator(SerializePointer(in.iterator));
}

void IteratorGetDistinguishedPointsTime::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void IteratorGetDistinguishedPointsTime::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(in.iterator(), pointer_map);
  auto const result = interface::principia__IteratorGetDistinguishedPointsTime(iterator);
  PRINCIPIA_CHECK_EQ(message.return_().result(), result);
}
...
void IteratorGetPlottableElements::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_iterator(SerializePointer(in.iterator));
}

void IteratorGetPlottableElements::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializePlottableElements(result);
}

void IteratorGetPlottableElements::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(in.iterator(), pointer_map);
  auto const result = interface::principia__IteratorGetPlottableElements(iterator);
  PRINCIPIA_CHECK_EQ(DeserializePlottableElements(message.return_().result(), pointer_map), result);
}
...
void PartAngularMomentumFromAngularVelocity::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
...
void PartAngularMomentumFromAngularVelocity::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_result() = SerializeXYZ(result);
}

void PartAngularMomentumFromAngularVelocity::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto world_angular_velocity = DeserializeXYZ(in.world_angular_velocity(), pointer_map);
  auto moments_of_inertia_in_tonnes = DeserializeXYZ(in.moments_of_inertia_in_tonnes(), pointer_map);
  auto principal_axes_rotation = DeserializeWXYZ(in.principal_axes_rotation(), pointer_map);
  auto part_rotation = DeserializeWXYZ(in.part_rotation(), pointer_map);
  auto const result = interface::principia__PartAngularMomentumFromAngularVelocity(world_angular_velocity, moments_of_inertia_in_tonnes, principal_axes_rotation, part_rotation);
  PRINCIPIA_CHECK_EQ(DeserializeXYZ(message.return_().result(), pointer_map), result);
}
...
void PartDragTorqueFromAngularVelocity::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_angular_drag_per_second(in.angular_drag_per_second);
  m->set_delta_t(in.delta_t);
  *m->mutable_world_angular_velocity() = SerializeXYZ(in.world_angular_velocity);
  *m->mutable_moments_of_inertia_in_tonnes() = SerializeXYZ(in.moments_of_inertia_in_tonnes);
  *m->mutable_principal_axes_rotation() = SerializeWXYZ(in.principal_axes_rotation);
...
void PartDragTorqueFromAngularVelocity::Fill(Return const& result, not_null<Message*> const message) {
  *message->mutable_return_()->mutable_torque_in_kilonewton_metre() = SerializeXYZ(result);
}

void PartDragTorqueFromAngularVelocity::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto angular_drag_per_second = in.angular_drag_per_second();
  auto delta_t = in.delta_t();
  auto world_angular_velocity = DeserializeXYZ(in.world_angular_velocity(), pointer_map);
  auto moments_of_inertia_in_tonnes = DeserializeXYZ(in.moments_of_inertia_in_tonnes(), pointer_map);
  auto principal_axes_rotation = DeserializeWXYZ(in.principal_axes_rotation(), pointer_map);
  auto part_rotation = DeserializeWXYZ(in.part_rotation(), pointer_map);
  auto const result = interface::principia__PartDragTorqueFromAngularVelocity(angular_drag_per_second, delta_t, world_angular_velocity, moments_of_inertia_in_tonnes, principal_axes_rotation, part_rotation);
  PRINCIPIA_CHECK_EQ(DeserializeXYZ(message.return_().torque_in_kilonewton_metre(), pointer_map), result);
}
...
void PluginReaderAwait::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_reader(SerializePointer(*in.reader));
}

void PluginReaderAwait::Fill(Out const& out, not_null<Message*> const message) {
  message->mutable_out()->set_reader(SerializePointer(*out.reader));
}

void PluginReaderAwait::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(SerializePointer(result));
}

void PluginReaderAwait::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto reader = DeserializePointer<PluginReader*>(in.reader(), pointer_map);
  [[maybe_unused]] auto const& out = message.out();
  auto const result = interface::principia__PluginReaderAwait(&reader);
  Delete(in.reader(), pointer_map);
  Insert(message.return_().result(), result, pointer_map);
}

void PluginReaderGet::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_reader(SerializePointer(*in.reader));
}

void PluginReaderGet::Fill(Out const& out, not_null<Message*> const message) {
  message->mutable_out()->set_reader(SerializePointer(*out.reader));
}

void PluginReaderGet::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(SerializePointer(result));
}

void PluginReaderGet::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto reader = DeserializePointer<PluginReader*>(in.reader(), pointer_map);
  [[maybe_unused]] auto const& out = message.out();
...
void PluginReaderLogs::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_reader(SerializePointer(in.reader));
}

void PluginReaderLogs::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void PluginReaderLogs::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto reader = DeserializePointer<PluginReader*>(in.reader(), pointer_map);
  auto const result = interface::principia__PluginReaderLogs(reader);
  PRINCIPIA_CHECK_EQ(message.return_().result().c_str(), result);
}

void PluginReaderWillBeSlow::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_reader(SerializePointer(in.reader));
}

void PluginReaderWillBeSlow::Fill(Return const& result, not_null<Message*> const message) {
  message->mutable_return_()->set_result(result);
}

void PluginReaderWillBeSlow::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto reader = DeserializePointer<PluginReader const*>(in.reader(), pointer_map);
  auto const result = interface::principia__PluginReaderWillBeSlow(reader);
  PRINCIPIA_CHECK_EQ(message.return_().result(), result);
}
