...
void CelestialWorldDegreesOfFreedom::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  auto plugin = DeserializePointer<Plugin const*>(pointer_map, in.plugin());
  auto index = in.index();
  auto const result = interface::principia__CelestialWorldDegreesOfFreedom(plugin, index);
  CHECK(DeserializeQP(message.return_().result()) == result);
}
...
void FreeVesselsAndPartsAndCollectPileUps::Run(Message const& message, Player::PointerMap& pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin*>(pointer_map, in.plugin());
  interface::principia__FreeVesselsAndPartsAndCollectPileUps(plugin);
}
...
void GetPartActualDegreesOfFreedom::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  auto const result = interface::principia__GetPartActualDegreesOfFreedom(plugin, part_id);
  CHECK(DeserializeQP(message.return_().result()) == result);
}
...
void InsertOrKeepVessel::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  auto const& out = message.out();
  bool inserted;
  interface::principia__InsertOrKeepVessel(plugin, vessel_guid, parent_index, loaded, &inserted);
}
...
void InsertOrKeepLoadedPart::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  interface::principia__InsertOrKeepLoadedPart(plugin, part_id, mass_in_tonnes, vessel_guid, main_body_index, main_body_world_degrees_of_freedom, part_world_degrees_of_freedom);
}
...
void InsertUnloadedPart::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  interface::principia__InsertUnloadedPart(plugin, part_id, vessel_guid, from_parent);
}
...
void IsKspStockSystem::Run(Message const& message, Player::PointerMap& pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin*>(pointer_map, in.plugin());
...
void IteratorGetQP::Run(Message const& message, Player::PointerMap& pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(pointer_map, in.iterator());
...
void IteratorGetTime::Run(Message const& message, Player::PointerMap& pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(pointer_map, in.iterator());
...
void IteratorGetXYZ::Run(Message const& message, Player::PointerMap& pointer_map) {
  auto const& in = message.in();
  auto iterator = DeserializePointer<Iterator const*>(pointer_map, in.iterator());
