...
void AdvanceParts::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_plugin(SerializePointer(in.plugin));
  m->set_t(in.t);
}

void AdvanceParts::Run(Message const& message, Player::PointerMap& pointer_map) {
  auto const& in = message.in();
  auto plugin = DeserializePointer<Plugin*>(pointer_map, in.plugin());
  auto t = in.t();
  interface::principia__AdvanceParts(plugin, t);
}
...
void InsertOrKeepLoadedPart::Run(Message const& message, Player::PointerMap& pointer_map) {
  ...
  interface::principia__InsertOrKeepLoadedPart(plugin, part_id, name, mass_in_tonnes, vessel_guid, main_body_index, main_body_world_degrees_of_freedom, part_world_degrees_of_freedom);
}
