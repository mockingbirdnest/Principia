...
void SetAngularMomentumConservation::Fill(In const& in, not_null<Message*> const message) {
  message->mutable_in()->set_conserve_angular_momentum(in.conserve_angular_momentum);
}

void SetAngularMomentumConservation::Run(Message const& message, Player::PointerMap& pointer_map) {
  [[maybe_unused]] auto const& in = message.in();
  auto conserve_angular_momentum = in.conserve_angular_momentum();
  interface::principia__SetAngularMomentumConservation(conserve_angular_momentum);
}

void VesselGetPileUpTrace::Fill(In const& in, not_null<Message*> const message) {
  auto* const m = message->mutable_in();
  m->set_plugin(SerializePointer(in.plugin));
  m->set_vessel_guid(in.vessel_guid);
}

void VesselGetPileUpTrace::Fill(Return const& result, not_null<Message*> const message) {
