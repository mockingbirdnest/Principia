...
MultiLevelSingleLinkage<Scalar, Argument>::FindGlobalMinima(
  ...
  //TODO(phl):parameters
  PrincipalComponentPartitioningTree<Argument> points_pcp_tree(
      /*values=*/{},
  ...
  //TODO(phl):parameters
  PrincipalComponentPartitioningTree<Argument> stationary_points_pcp_tree(
      /*values=*/{},
      /*max_values_per_cell=*/10);
  ...
  for (std::int64_t k = 1; k <= number_of_rounds; ++k) {
    ...
    std::vector<Argument> pointsₖ = RandomArguments(box_, N);
    for (auto& pointₖ : pointsₖ) {
      points.push_back(std::move(pointₖ));
      points_pcp_tree.Add(&points.back());
    }

    auto const rₖ = CriticalRadius(/*σ=*/4, /*kN=*/k * N);

    for (auto& xᵢ : points) {
      auto* const xⱼ = points_pcp_tree.FindNearestNeighbour(
      ...
      if (xⱼ == nullptr || (xᵢ - *xⱼ).Norm() > rₖ) {
        ...
        // Use the stationary point PCP to determine if this is a new stationary
        // point or not.  The factor 2 below is because we are looking at the
        // distance between two approximate stationary points, not between an
        // approximate stationary point and the real one.
        auto* const nearest_neighbour_of_stationary_point =
            stationary_points_pcp_tree.FindNearestNeighbour(stationary_point);
        if (nearest_neighbour_of_stationary_point == nullptr ||
            (stationary_point - *nearest_neighbour_of_stationary_point)
                    .Norm() > 2 * local_search_tolerance) {
          stationary_points.push_back(
              std::make_unique<Argument>(stationary_point));
          stationary_points_pcp_tree.Add(stationary_points.back().get());
        }
      }
    }
  }
