...
TEST(ApproximationTest, 4543) {
  // A discontinuous function.  The interpolation will zoom on the discontinuity
  // and give up.
  auto const f = [](double const x) { return x > 1.0 ? 2.0 : -3.0; };
  SubdivisionPredicate<double, double> subdivide =
      [](auto const& _, double const& error_estimate) -> bool {
    return true;
  };
  ...
  StreamingAdaptiveЧебышёвPolynomialInterpolant<8>(f,
                                                   ...
                                                   stop,
                                                   &error_estimate);
  // We can't build a great approximation.  The error is roughly the size of the
  // discontinuity.
  EXPECT_THAT(error_estimate, IsNear(4.6_(1)));
  EXPECT_EQ(56, number_of_interpolants);
}
