...
(*With[{indices=Flatten[Position[ListQ/@Table[sincos`accurateTables[i],{i,0,500}],True]]-1},*)
(*"#pragma once*)
(**)
(*#include <limits>*)
(**)
...
(*};*)
(**)
(*constexpr std::array<SinCosAccurateValues, " <> ToString[Max[indices]] <> "> SinCosAccurateTable{\n" <>*)
(*StringRiffle[Join[Table["    {.x =     std::numeric_limits<double>::signaling_NaN(),\n     .sin_x = std::numeric_limits<double>::signaling_NaN(),\n     .cos_x = std::numeric_limits<double>::signaling_NaN()",{i,0,Min[indices]-1}],Table["    {.x =     " <> hexFloatLiteral[sincos`accurateTables[i][[1]],False] <> ",\n     .sin_x = " <> hexFloatLiteral[sincos`accurateTables[i][[2]],False] <> ",\n     .cos_x = "<> hexFloatLiteral[sincos`accurateTables[i][[3]],False], {i,indices}]],"},\n"]<>"}};*)
(**)
(*}  // namespace internal*)
(**)
